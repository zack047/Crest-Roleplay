...
<div class="newdonate__info ">
    <div class="newdonate__info-block" on:mouseenter on:mouseleave>
        <div class="newdonate__info-number">
            <input type="text" class="newdonate__number_input" placeholder="MetaverseRP" bind:value={number} maxLength={8}  on:focus={() => confirm = false}>
        </div>
        <div class="newdonate__info-info">
            <div class="box-flex">
                <div class="newdonate__info-title">Transport Number</div>
            </div>
            <div class="newdonate__info-paragraph">
                Click on the picture on the left and enter the desired number. The number must not start with 0. <br><br><b>{getPrice (number)[2]}</b> The vehicle number will allow you to attract envious galnces of others to your car! The price depends on the number of characters and the number format.
            </div>
        </div>
        {#if !confirm}
