<script>
    ...
    const onDonate = () => {
        ...
        //    return window.showTooltip("#donateInput", 2, "What went wrong...");
        if (donateText < 0 || donateText > 999999)
            return window.showTooltip("#donateInput", 2, "Invalid Format");
        donateToggled = false;
        const config = {
            headers: {
...
<div class="newdonate__payment">
    <div class="newdonate__payment-block" on:mouseenter on:mouseleave>
        ...
        <div class="newdonate__payment-title">Replenish CC</div>
        <input class="newdonate__input margin-bottom-22" placeholder="Sum" type="text" bind:value={donateText} on:input={(event) => onHandleInput (event.target.value)}>
        <div class="newdonate__input">
            <span class="redbucks-small-img"/>
            <div>{getDonate(donateText)} CC</div>
        </div>
        <div class="newdonate__payment-description">You will recieve</div>
        <div class="newdonate__payment-subtitle">Choose a payment system</div>
        ...
        <div class="newdonate__button_small">
            <div class="newdonate__button-text" on:click={onDonate}>Further</div>
        </div>
    </div>
