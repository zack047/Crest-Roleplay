<script>
    ...
    const onSelectMenu = () => {
        getUnique ();
        if (isBuy)
            return window.notificationAdd(4, 9, `You have already used this offer`, 3000);
...
<div class="main-menu__header-element" class:isBuy={isBuy}>
    ...
    <div class="main-menu__element-info">
        <div class="main-menu__timer-box">
            <div class="main-menu__timer box-center">Offer of the day (-30%)</div>
        </div>
        ...
        <div class="newdonate__button_small" on:click={onSelectMenu}>
            <div class="newdonate__button-text">Buy for {format("money", Math.round (selectListId.price * 0.7))} CC</div>
        </div>
    </div>
</div>
{/if}