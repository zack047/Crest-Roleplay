<script>
    ...
    const onSlotUse = (sSlot, index) => {
        ...
        if (SlotListToggled[sSlot][index] === true) {
            ...
            onGetWin (-2);
            window.notificationAdd(4, 9, `Unfortunately, you lost.`, 3000);
        }
    ...
    const onStartGame = () => {
        if (IsGame !== false) return;
            ...
            window.notificationAdd(4, 9, `You dont have enough CC!`, 3000);
            return;
        } else if (Number (inputCash) < betMin) {            
            inputCash = betMin;
            window.notificationAdd(4, 9, `The minimum bid is ${format("money", betMin)} CC`, 3000);
            return;
        } else if (Number (inputCash) > Number (betMax)) {            
            inputCash = betMax;
            window.notificationAdd(4, 9, `The maximum bid is ${format("money", betMax)} CC`, 3000);
            return;
        }
        //IsGame = true;//
        //SelectSlot = 0;//
    ...
    const startGame = (bet) => {
        ...
        IsGame = true;//
        SelectSlot = 0;//
        window.notificationAdd(4, 9, `The game has started`, 3000);
    }
    ...
    const onGetWin = (type) => {
        if (type > 0) window.notificationAdd(4, 9, `The game is over, you have won ${format("money", type < 1 ? 0 : Math.round (inputCash * SlotListCoef [type - 1]))} CC`, 3000);
    ...
    <div class="game">
        <div class="box">
            ...
            <div class="content">
                ...
                <div class="title"><span>Minesweeper</span></div>
                <p class="desc"> Minesweeper game - Test your luck, moving towards a bigger prize with each step!</p>
                <div class="title rb">Your Balance: <span>{format("money", $accountRedbucks)} CC</span></div>
                

                <div class="win">
                    <div class="n">{format("money", SelectSlot < 1 ? 0 : Math.round (inputCash * SlotListCoef [SelectSlot - 1]))} CC</div>
                    <p>'Your Winnings'</p>
                </div>
                ...
                <div class="input">
                    ...
                    {#if IsGame == false}
                    <div class="btn" on:click={onStartGame}>Play</div>
                    {:else}                    
                    <div class="btn" on:click={() => onGetWin (SelectSlot)}>Pick up</div>
                    {/if}
                    <div class="exit" on:click={onExit}>Exit</div>
                </div>
            </div>
