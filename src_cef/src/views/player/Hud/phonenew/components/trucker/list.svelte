                ...
                <div class="newphone__maps_client">
                    <div class="box-between">
                        <div class="box-column">
                            ...
                            <div class="gray">Distance {order.dist} M.</div>
                            <div>Item: <span class="violet">{order.name}</span></div>
                            <div>You will get: <span class="green">${format("money", order.price)}</span></div>
                        </div>
                        <div class="phoneicons-Button newphone__chevron-back"></div>
                    </div>
                    <div class="newphone__client_button" on:click={() => onTakeOrder (order.uid)}>Take an order</div>
                </div>
            {/each}
            <div class="newphone__project_button" on:click={() => onLoad (10 + countLoad)}>Load orders</div>
        {:else}
            <div class="newphone__forbes_anon newphone__project_padding20">
                <div class="newphone__forbes_hidden noneelements"></div>
                <div class="gray">There are no active orders. But there will be something soon...</div>
            </div>
        {/if}
    </div>
</div>