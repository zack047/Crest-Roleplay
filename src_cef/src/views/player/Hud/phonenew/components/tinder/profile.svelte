<script>
    ...
    const onSave = () => {
        if (avatar == true) {
            return window.notificationAdd(4, 9, "The photo is being uploaded", 3000);
        }
...
<div class="newphone__ads_list auction big">
    <div class="tinder--card">
        ...
        <!--<div class="tinder__name gray newphone__project_padding16">13 km. von Ihnen entfernt</div>-->
        <div class="tinder__line"></div>
        <div class="tinder__descr newphone__project_padding16">{profile.text}</div>
    </div>
    <div class="newphone__project_button tinder" on:click={onEdit}>edit</div>
</div>
{:else}
    <div class="newphone__project_padding16">
        ...
        <textarea placeholder="Enter message text..." class="newphone__message_textarea profile" bind:value={text} maxlength={maxSymbol} on:focus={onInputFocus} on:blur={onInputBlur}></textarea>
        <div class="newphone__maps_headertitleGPS">I am looking for:</div>
        ...
        <div class="newphone__project_button tinder" on:click={onSendImage}>Select photo</div>
        <div class="newphone__project_button tinder" on:click={onSave}>Save</div>
    </div>
{/if}
