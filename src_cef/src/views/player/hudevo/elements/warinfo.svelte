...
<div class="hudevo__event" in:fly={{ x: 50, duration: 500 }} out:fly={{ x: 50, duration: 250 }}>
    <div class="hudevo__notification_subtitle">Warfare</div>
    <div class="hudevo__notification_title">
        {isAttack ? 'Ты выиграл войну' : 'Ты проиграл войну'}
    </div>
    <div class="hudevo__notification_text medium">
        {#if mapName && mapName.length > 1}
            <div>Зона: <b>{mapName}</b></div>
        {/if}
        <div>Тип сражения: <b>{typeBattle[selectTypeBattle]}</b></div>
        <div>Время: <b>{time}</b></div>
        <div>Количество: <b>{selectComposition == -1 ? composition [0] : composition [selectComposition] }</b></div>
