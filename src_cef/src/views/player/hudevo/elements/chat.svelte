...
<div class="hudevo__chat-block" style="height: {heightChat}px; margin-top: {SafeSone.y}px; margin-left: {SafeSone.x}px;">
    <div class="hudevo__chat" class:noactive={noActive} style="opacity: {!noActive ? 1 : 1 / 100 * Alpha}" bind:clientHeight={heightChat}>
        <div class="hudevo__chat_messages"
            style="width: 100%;font-size: 1.481vhpx;line-height: 1.852vh;height: {MessageHeight * PageSize}px;transition: opacity {1 / 100 * Transition}s;overflow-y: {$isInputToggled ? 'auto' : 'hidden'};" class:scroll={(Messages.length > pagesize ? (!$isInputToggled ? false : true) : false)}>
        ...
        {#if $isInputToggled}
            <div class="hudevo__chat_input_block">
                <div class="hudevo__chat_input_box">
                    ...
                    <input class="hudevo__chat_input" bind:this={TextInput} spellCheck={false} maxLength={144} placeholder="Enter your message" bind:value={InputValue} />
                    <div class="hudevo__chat_input_send" on:keypress={() => {}} on:click={OnSubmitMessage}>
            ...
            <div class="hudevo__chat_buttons_block">
                <div class="hudevo__chat_buttons">
                    ...
                    {#each ChatTegs as tag, index}
                        {#if tag.visible}
                        <div class="hudevo__chat_button" class:active={index === selectChat} on:keypress={() => {}} on:click={() => selectChat = index }>{tag.name}</div>
                        {/if}
                    {/each}
                </div>
            </div>
