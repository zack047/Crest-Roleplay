<script>
    ...
    //messages = JSON.parse(`[{"Number":41570,"Avatar":"https://api.exsight-rp.com/image/11_121/23/1640267825898.jpg","Name":"Vitala","Date":"2021-12-23T19:37:28","Text":"С…РµР№ С…РµР№ С…Р° СЌС‚Рѕ РґРёСЃСЃ РЅР° РїРµС‚СѓС…Р° С…Р° С…Р°","Type":0,"IsMe":true,"Status":true}]`)
...
<div class="box-center box-between w-1 newphone__project_padding16" in:fade>
    ...
    <div class="newphone__contacts_circle" on:keypress={() => {}} on:click={() => isPopup = true}>+</div>
</div>
<div class="newphone__contacts_inputblock newphone__project_padding16" in:fade>
    <div class="phoneicons-loop"></div>
    <input type="text" class="newphone__contacts_input" placeholder="Search" bind:value={searchText} on:focus={onInputFocus} on:blur={onInputBlur}>
</div>
<div class="newphone__messages_list" in:fade>
    ...
    {#if messages && typeof messages === "object" && messages.length}
        {#each messages.filter(el => filterCheck(el, searchText)) as message}
            <div class="newphone__messages_element" on:keypress={() => {}} on:click={() => onSelectNumber(message.Number)}>
