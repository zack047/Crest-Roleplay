...
<div class="newphone__messages_smiles">
    <div class="newphone__smiles_selector">
        {#each CategoryList as category, index}
            <div class="{category.icon} newphone__smiles_select" class:active={activeCategory === category.index} on:keypress={() => {}} on:click={() => onSelectCategory (category.index)}></div>
        {/each}
    </div>
    <div class="newphone__smiles_input">
        <div class="phonesmiles-loop"></div>
        <input type="text" placeholder="Search by emoji ..." bind:value={searchText} on:focus={onInputFocus} on:blur={onInputBlur}>
    </div>
    ...
    <div class="newphone__smiles_info">
        ...
        {#if isLeave}
            ...
            <div class="newphone__smiles_skins">
                <div class="newphone__smiles_skin {colorsList [selectedColor]}" on:keypress={() => {}} on:click={() => updateToggledColor (true)}></div>
            </div>
            {:else}
                <div class="newphone__smiles_skins">
                    {#each colorsList as color, index}
                        <div class="newphone__smiles_skin {color} mr-3" class:active={selectedColor === index} on:keypress={() => {}} on:click={() => onSelectColor (index)}></div>
                    {/each}
                </div>
            {/if}
        {/if}
