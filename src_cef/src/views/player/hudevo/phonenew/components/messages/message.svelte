...
<div class="box-center box-between w-1 newphone__project_padding16" in:fade>
    <div class="phoneicons-Button" on:keypress={() => {}} on:click={() => onSelectNumber(-1)}></div>
    ...
    <div class="newphone__settings_bigicon n-m" on:keypress={() => {}} on:click={onContact} class:phoneicons-contacts={!getAvatar(contactData.Avatar).length} use:loadImage={getAvatar(contactData.Avatar)}></div>
</div>
<div class="newphone__messages_chat" on:scroll={OnScroll} bind:this={chatElement} in:fade>
    {#each messages as message}
        <div class="newphone__messages_sms" class:me={message.Me} class:map={message.Type === messageType.map} class:image={message.Type === messageType.img} on:keypress={() => {}} on:click={() => onMessage (message)}>
            ...
            {:else if message.Type === messageType.img}
                <img alt='' class="message__img" src='' use:loadImage={message.Text} />
            {/if}
...
<div class="newphone__messages_bottom newphone__project_padding16" in:fade>
    ...
    <div class="phoneicons-add1 newphone__messages_smallicon m-0" on:keypress={() => {}} on:click={openPopup}></div>
    {/if}
    <input type="text" class="newphone__messages_input" placeholder={`Write ${contactData.Name}`} bind:value={inputValue} maxlength={inputMaxLength} bind:this={inputDiv} on:focus={onFocus} on:blur={onBlur} on:input={onInput} />
    {#if !contactData.IsSystem}
    <div class="phoneicons-smile newphone__messages_smallicon m-0 mr-6" on:keypress={() => {}} on:click={()=> isSmile = !isSmile}></div>
    {/if}
    {#if isSmile}
        <EmojiIndex {addSmile} />
    {/if}
    <div class="phoneicons-send1 newphone__messages_smallicon m-0 small" on:keypress={() => {}} on:click={() => onSend (messageType.text)}></div>
</div>