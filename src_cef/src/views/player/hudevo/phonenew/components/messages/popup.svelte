...
<div class="newphone__popup" in:fade>
    <div class="newphone__popup_element" on:keypress={() => {}} on:click={sendGeo}>{translateText('player2', 'Отправить геопозицию')}</div>
    <div class="newphone__popup_element" on:keypress={() => {}} on:click={onSendImage}>{translateText('player2', 'Отправить фото')}</div>

    {#if contactData.IsBlackList}
        <div class="newphone__popup_element red" on:keypress={() => {}} on:click={onDellBlackList}>{translateText('player2', 'Разблокировать контакт')}</div>
    {:else}
        <div class="newphone__popup_element" on:keypress={() => {}} on:click={onAddBlackList}>{translateText('player2', 'Заблокировать контакт')}</div>
    {/if}

    <div class="newphone__popup_element" on:keypress={() => {}} on:click={onClosePopup}>{translateText('player2', 'Отмена')}</div>
</div>