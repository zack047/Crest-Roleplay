...
<div class="newphone__rent_list" in:fade>
   ...
   <!--{#if selectedCar.time == "Аренда"}
    ...
    {#if selectedCar.isRent}
        <div on:keypress={() => {}} on:click={() => onEnterRent ("gpstrack")} class="newphone__project_button rent">{translateText('player2', 'Показать на карте')}</div>
        {#if !selectedCar.isJob}
        <div on:keypress={() => {}} on:click={() => onEnterRent ("datetime")} class="newphone__project_button rent">{translateText('player2', 'Продлить время аренды за')} ${format("money", selectedCar.rentPrice)}</div>
        {/if}
        <div on:keypress={() => {}} on:click={() => onEnterRent ("stoprent")} class="newphone__project_button rent">{translateText('player2', 'Отказаться от аренды')}</div>
    {:else}
        {#each functionData as func}
            {#if isVisible (func, selectedCar, inGarage)}
                <div on:keypress={() => {}} on:click={() => onEnter (func, selectedCar)} class="newphone__project_button rent">
                    {func.name}
                    {#if func.func == "sell"}
        ...
        {/each}
    {/if}
    <div class="orange box-center m-top10" on:keypress={() => {}} on:click={() => OnUpdatePage ("List")}>{translateText('player2', 'Назад')}</div>
</div>