...
<div in:fade class="box-100">
    ...
    <div class="newphone__ads_list filter">
        {#each categorieName as item}
            <div class="box-between w-100" on:keypress={() => {}} on:click={() => onUpdateFilter (item)}>
                <div>{item}</div>
    ...
    <div class="newphone__project_button rentnm" on:keypress={() => {}} on:click={() => updateFilter (filter)}>{translateText('player2', 'Применить фильтр')}</div>
    <div class="orange box-center" on:keypress={() => {}} on:click={closeFilter}>{translateText('player2', 'Назад')}</div>
</div>