...
{:else}
    {#if carsList && typeof carsList === "object" && carsList.length > 0}
        <div class="box-between w-100 mb-10 newphone__project_padding20">
            <input type="text" class="newphone__ads_input small170 mb-0 mr-6" placeholder="Search..." bind:value={inputText} on:focus={onInputFocus} on:blur={onInputBlur}>
            {#if !isTypeFilter.length}
                <div class="newphone__button_filter box-center" on:keypress={() => {}} on:click={openFilter}>
                    <span class="phoneicons-filter"></span>
                    {translateText('player2', 'Фильтры')}
                </div>
            {:else}
                <div class="newphone__button_filter box-center orange__background" on:keypress={() => {}} on:click={openFilter}>
                    {isTypeFilter.length} {getFilterWord(isTypeFilter.length)}
                    <span class="phoneicons-close" on:keypress={() => {}} on:click={clearFilter}></span>
                </div>
            {/if}
        </div>
        <div class="newphone__rent_list small" in:fade>
            {#each carsList.filter((el) => isFilter(el, inputText)) as item}
                ...
                <div class="newphone__rent_none hover vehicle" on:keypress={() => {}} on:click={() => OnUpdatePage("Car", item)}>
    ...
    {:else}
        ...
        <div class="newphone__project_button rent" on:keypress={() => {}} on:click={setPointArenda}>{translateText('player2', 'Найти аренду')}</div>
        <div class="newphone__project_button rent" on:keypress={() => {}} on:click={()=> currentPage.set("taxi")}>{translateText('player2', 'Вызвать такси')}</div>
    {/if}
{/if}