    ...
    <div class="newphone__maps_subcategories" bind:this={otherElement} use:setOtherElement>
        ...
        <div class="newphone__maps_list" style="height: auto; min-height: auto; max-height: auto;">
            {#if selectMech && selectMech.isOrder}
                ...
                <div class="newphone__project_button" on:keypress={() => {}} on:click={onCancelOrder}>{translateText('player2', 'Отменить')}!</div>
            {:else}
                <div class="newphone__project_button" on:keypress={() => {}} on:click={onOrder}>{translateText('player2', 'Заказать')}</div>
            {/if}
            <div class="mechcolor box-center" on:keypress={() => {}} on:click={closeMenu}>{translateText('player2', 'Закрыть')}</div>
        </div>
        <HomeButton />
    </div>
</div>