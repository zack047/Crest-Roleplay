...
<div in:fade class="box-100">
    ...
    <div class="newphone__maps_list">
        ...
        <div class="newphone__project_button mt-0" on:keypress={() => {}} on:click={onStartPoint}>{translateText('player2', 'Показать на карте')}</div>
        <div class="newphone__project_button mt-0" on:keypress={() => {}} on:click={onCancelOrder}>{translateText('player2', 'Отменить заказ')}</div>
    </div>
</div>