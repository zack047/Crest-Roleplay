                ...
                <div class="newphone__maps_client">
                    <div class="box-between">
                        <div class="box-column">
                            ...
                            <div>Goods: <span class="violet">{order.name}</span></div>
                            <div>You'll get: <span class="green">${format("money", order.price)}</span></div>
                        </div>
                        <div class="phoneicons-Button newphone__chevron-back"></div>
                    </div>
                    <div class="newphone__client_button" on:keypress={() => {}} on:click={() => onTakeOrder (order.uid)}>Take an order</div>
                </div>
            {/each}
            <div class="newphone__project_button" on:keypress={() => {}} on:click={() => onLoad (10 + countLoad)}>Download orders</div>
        {:else}
