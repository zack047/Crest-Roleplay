...
{#if list.length}
    <div class="tinder" class:tinder_love={status === "love"} class:tinder_nope={status === "nope"}>
        ...
        <div class="tinder--buttons">
          <button id="nope"><i class="tindericons-mark" on:keypress={() => {}} on:click={() => onAction (false)}></i></button>
          <button id="love"><i class="tindericons-heart" on:keypress={() => {}} on:click={() => onAction (true)}></i></button>
        </div>
    </div>
{:else}
    <div class="newphone__forbes_anon newphone__project_padding20">
        <div class="newphone__forbes_hidden"></div>
        <div class="gray">It seems that there is no one here .. but soon someone will appear!</div>
    </div>
{/if}