...
<div class="newphone__forbes" in:fade>
    ...
    <div class="newphone__forbes_content">
        ...
        <div class="box-between newphone__project_padding16 mb-10">
            {#if isCreate}
                <span class="tindericons-users tinder__button" on:keypress={() => {}} on:click={() => selectedView = "List"}></span>
            {:else}
                <span/>
            {/if}
            <div class="box-flex">
                {#if isCreate}<span class="tindericons-heart tinder__button mr-6" on:keypress={() => {}} on:click={() => selectedView = "Matches"}></span>{/if}
                <span class="tindericons-person tinder__button" on:keypress={() => {}} on:click={() => selectedView = "Profile"}></span>
            </div>
        </div>
