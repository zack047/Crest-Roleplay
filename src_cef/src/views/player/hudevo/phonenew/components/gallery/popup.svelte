...
<div class="newphone__popup b-2" in:fade>
    {#if !useImage}
        <div class="newphone__popup">
            <div class="newphone__popup_element" on:keypress={() => {}} on:click={onCamera}>{translateText('player2', 'Сделать фото')}</div>
            <div class="newphone__popup_element" on:keypress={() => {}} on:click={() => useImage = true}>{translateText('player2', 'Выбрать из галереи')}</div>
            <div class="newphone__popup_element red" on:keypress={() => {}} on:click={onCancel}>{translateText('player2', 'Отмена')}</div>
        </div>
    {:else}
        <div class="newphone__gallery newphone__popup_border">
            <div class="box-between w-1 newphone__project_padding20 pt-20">
                ...
                <div class="phoneicons-add1" on:keypress={() => {}} on:click={() => useImage = false}></div>
            </div>
            {#if data.length}
            <div class="newphone__gallery_grid">
                {#each data as item}
                    <div class="newphone__gallery_item" use:loadImage={item[0]} on:keypress={() => {}} on:click={() => selectImage (item[0])} />
                {/each}
            </div>
            {:else}
