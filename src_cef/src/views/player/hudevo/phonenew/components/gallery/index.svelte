...
<div class="newphone__gallery" style={selectedImage ? "background: linear-gradient(180deg, rgba(180,180,180,1) 0%, rgba(247,247,239,1) 10%)" : "background: #FEFEFE"} in:fade>
    ...
    {#if selectedImage == null}
        ...
        <div class="newphone__gallery_grid">
            {#each data as item}
                <div class="newphone__gallery_item" use:loadImage={item[0]} on:keypress={() => {}} on:click={() => selectedImage = item} />
            {/each}
        </div>
    ...
    {:else}
        {#if isDeletePopup}
            <div class="newphone__popup">
                <div class="newphone__popup_element red" on:keypress={() => {}} on:click={() => onDell (selectedImage[0])}>{translateText('player2', 'Удалить фото')}</div>
                <div class="newphone__popup_element" on:keypress={() => {}} on:click={() => isDeletePopup = false}>{translateText('player2', 'Отмена')}</div>
            </div>
        {/if}
        <div class="newphone__gallery_header">
            <div class="phoneicons-Button newphone__backicon" on:keypress={() => {}} on:click={() => selectedImage = null}></div>
        ...
        <div class="newphone__gallery_footer">
            <div class="newphone__gallery_last">
                {#each data as item}
                    <div class="newphone__galleryfooter_item" use:loadImage={item[0]} on:keypress={() => {}} on:click={() => selectedImage = item} />
                {/each}
            </div>
            <div class="box-between">
                <div class="phoneicons-upload" style="opacity:0"></div>
                <div class="phoneicons-trash"  on:keypress={() => {}} on:click={() => isDeletePopup = true}></div>
            </div>
        </div>
    {/if}
    <HomeButton />
