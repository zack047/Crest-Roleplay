...
<div class="newphone__call" in:fade>
    ...
    <div class="nephone__call_selectors">
        <div class="box-between w-1">
            <div class="newphone__call_selector" class:active={$currentView === "recent"} on:keypress={() => {}} on:click={() => updateView ("recent")}>
                <div class="phoneicons-recent newphone__selector_icon"></div>
                <div>{translateText('player2', 'Недавние')}</div>
            </div>
            <div class="newphone__call_selector" class:active={$currentView === "contacts"} on:keypress={() => {}} on:click={() => updateView ("contacts")}>
                <div class="phoneicons-contacts newphone__selector_icon"></div>
                <div>{translateText('player2', 'Контакты')}</div>
            </div>
            <div class="newphone__call_selector" class:active={$currentView === "dial"} on:keypress={() => {}} on:click={() => updateView ("dial")}>
