...
{#if !isEdit}
    ...
    <div class="box-between box-center w-1 newphone__project_padding16">
        <div class="newphone__contacts_box" on:keypress={() => {}} on:click={() => onMessage (contactData.Number)}>
            <div class="phoneicons-chat newphone__contacts_but"></div>
            <div>{translateText('player2', 'Написать')}</div>
        </div>
        {#if !contactData.IsSystem}
        <div class="newphone__contacts_box" on:keypress={() => {}} on:click={() => onCall (contactData.Number)}>
    ...
    {#if !contactData.IsSystem}
        {#if !contactData.IsAdded}
            <div class="newphone__contacts_button blue" on:keypress={() => {}} on:click>{translateText('player2', 'Добавить контакт')}</div>
        {/if}
        {#if contactData.IsBlackList}
            <div class="newphone__contacts_button blue" on:keypress={() => {}} on:click={onDellBlackList}>
                {translateText('player2', 'Разблокировать контакт')}
            </div>
        {:else}
            <div class="newphone__contacts_button blue" on:keypress={() => {}} on:click={onAddBlackList}>
                {translateText('player2', 'Заблокировать контакт')}
            </div>
        {/if}
        {#if contactData.IsAdded}
        <div class="newphone__contacts_button red" on:keypress={() => {}} on:click={onDellContact}>{translateText('player2', 'Удалить контакт')}</div>
        {/if}
    {/if}
{:else}
    <div class="w-1 box-between newphone__project_padding16" in:fade>
        <div></div>
        <div class="phoneicons-add1" on:keypress={() => {}} on:click={updateEditStatus}></div>
    </div>
    <div class="newphone__contacts_bigicon hover" class:phoneicons-contacts={!getAvatar(avatarValue).length} style={getAvatar(avatarValue)} on:keypress={() => {}} on:click={onPopupImage}></div>
    ...
    <div class="newphone__addcontact_button" on:keypress={() => {}} on:click={onUpdateContact}>
