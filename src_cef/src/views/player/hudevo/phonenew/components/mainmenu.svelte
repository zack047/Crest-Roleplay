            ...
            if (pageName === "camera") {
                executeClientAsyncToGroup("getGallery").then((result) => {
                    ...
                    if (result.length >= 25) {
                        ...
                        window.notificationAdd(4, 9, `The place ended on the phone, first delete the photos`, 3000);
                    } else
...
<div class="newphone__background" in:fade="{{duration: 200}}" style="background-image: url({background})">
    ...
    <div class="newphone__mainmenu_grid">
        ...
        {#each menuArray as item}
            {#if item.jobId === undefined || item.jobId === $charWorkID}
                <div class="newphone__mainmenu_element" on:keypress={() => {}} on:click={() => onSelectPage(item.link)}>
    ...
    <div class="newphone__mainmenu_bottom">
        <div class="newphone__mainmenu_element" on:keypress={() => {}} on:click={() => onSelectPage("call")}>
            <div class="newphone__mainmenu_icon" style="background-image: url({CallIcon})"></div>
        </div>
        <div class="newphone__mainmenu_element" on:keypress={() => {}} on:click={() => onSelectPage("messages")}>
            <div class="newphone__mainmenu_icon" style="background-image: url({MessagesIcon})"></div>
        </div>
        <div class="newphone__mainmenu_element" on:keypress={() => {}} on:click={() => onSelectPage("camera")}>
            <div class="newphone__mainmenu_icon" style="background-image: url({CameraIcon})"></div>
        </div>
        <div class="newphone__mainmenu_element" on:keypress={() => {}} on:click={() => onSelectPage("settings")}>
