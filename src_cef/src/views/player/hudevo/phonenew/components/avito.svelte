...
<div class="newphone__rent">
    ...
    <div class="newphone__rent_content">
        ...
        {#if currentElement == "list"}
            <div class="newphone__project_button ravito" on:keypress={() => {}} on:click={()=> currentElement = "create"}>Submit an ad</div>
            <div class="box-center box-between newphone__project_padding20">
                <div class="newphone__maps_header m-0">ads:</div>
                {#if false}
        ...
        {#if currentElement == "create" && addSelector === null}
            <div class="newphone__maps_header m-top newphone__project_padding20">Submit an ad:</div>
            <div class="newphone__ads_list">
        ...
        {#if currentElement == "filter" && addSelector === null}
            <div class="newphone__maps_header m-top newphone__project_padding20">Filters:</div>
            ...
            <div class="box-between w-1 newphone__project_padding20">
                <div class="box-column">
                    <div class="newphone__ads_title newphone__project_padding20">Price from:</div>
                    <input type="text" class="newphone__ads_input small" placeholder="Enter the text" on:focus={onInputFocus} on:blur={onInputBlur}>
                </div>
                <div class="box-column">
                    <div class="newphone__ads_title newphone__project_padding20">Price up to:</div>
                    <input type="text" class="newphone__ads_input small" placeholder="Enter the text" on:focus={onInputFocus} on:blur={onInputBlur}>
                </div>
            </div>
            <div class="newphone__project_button ravito">Apply a filter</div>
            <div class="violet box-center" on:keypress={() => {}} on:click={goToStartPage}>Back</div>
        {/if}
        {#if addSelector}
            <div class="newphone__maps_header m-top newphone__project_padding20">{categorieName[addSelector]}</div>
            <div class="newphone__ads_title newphone__project_padding20">Title:</div>
            <input type="text" class="newphone__ads_input" placeholder="Enter the text" on:focus={onInputFocus} on:blur={onInputBlur}>
            <div class="newphone__ads_info small">
                <textarea class="newphone__ads_textarea" placeholder="Start entering a message"></textarea>
                <div class="newphone__ads_capture">
                    {#if false}
                        <div class="newphone__ads_gray">Add photos+</div>
                    {/if}
                    {#if true}
            ...
            <div class="newphone__ads_title newphone__project_padding20">Price:</div>
            <input type="text" class="newphone__ads_input" placeholder="Введите текст" on:focus={onInputFocus} on:blur={onInputBlur}>
            <div class="newphone__project_button ravito">Submit an ad</div>
            <div class="violet box-center" on:keypress={() => {}} on:click={goToStartPage}>Back</div>
        {/if}
    </div>
    <HomeButton />
