...
<div class="newphone__ads_list">
    {#if itemList && itemList.length > 0}
        {#each itemList as item, index}
            <div class="newphone__project_categorie" on:keypress={() => {}} on:click={()=> onSelect(item.id)}>
                <div>{item.name}</div>
...
<div class="green box-center mt-5" on:keypress={() => {}} on:click={() => onSelectedView("Create")}>{translateText('player2', 'Назад')}</div>