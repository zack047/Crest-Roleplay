    ...
    {#if list && list.length > 0}
        {#each list as item}
            <div class="newphone__ads_element">
                ...
                <div class="box-between w-1">
                    <div class="newphone__ads_gray green">Current rate:</div>
                ...
                {/if}
                <div class="newphone__project_button auction small" on:keypress={() => {}} on:click={()=> onSelect (item.id)}>{translateText('player2', 'Принять участие')}</div>
            </div>
        {/each}
    {:else}
...
<div class="green box-center mt-5" on:keypress={() => {}} on:click={()=> onSelectedView ("Main")}>{translateText('player2', 'На главную')}</div>