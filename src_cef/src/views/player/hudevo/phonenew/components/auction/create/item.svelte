...
<div class="newphone__ads_info small mt-5">
    ...
    <!--<div class="newphone__ads_capture">
        {#if !cameraLink}
            <div class="newphone__ads_gray" on:keypress={() => {}} on:click={updateCameraToggled}>Добавить фото+</div>
        {:else}
            {#if cameraLink !== true}
            <div class="newphone__ads_addimg" style="background-image: url({cameraLink})"></div>
            {/if}
            <div class="box-column">
                <span class="phoneicons-reload green" on:keypress={() => {}} on:click={updateCameraToggled}></span>
                <span class="phoneicons-trash green" on:keypress={() => {}} on:click={() => cameraLink = false}></span>
            </div>
        {/if}
    </div>-->
</div>
{#if isConfirmed === false}
    <div class="newphone__ads_title newphone__project_padding20">starting price:</div>
    <input type="text" class="newphone__ads_input" placeholder="Enter the price" on:focus={onInputFocus}
           on:blur={onInputBlur} bind:value={price}>
    <div class="newphone__project_button auction mb-0" on:keypress={() => {}} on:click={() => isConfirmed = true}>{translateText('player2', 'Выставить лот')}</div>
{:else}
    ...
    <div class="newphone__project_button auction mb-6" on:keypress={() => {}} on:click={onSend}>{translateText('player2', 'Я уверен')}</div>
    <div class="newphone__project_button auction mb-0" on:keypress={() => {}} on:click={() => isConfirmed = false}>{translateText('player2', 'Я не уверен')}</div>
{/if}
<div class="green box-center mt-5" on:keypress={() => {}} on:click={() => onSelectedView("CreateList")}>{translateText('player2', 'Назад')}</div>