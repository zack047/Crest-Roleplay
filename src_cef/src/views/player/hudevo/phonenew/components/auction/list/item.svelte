...
<div class="newphone__ads_list auction big">
    ...
    {#if !isBet}
        ...
        {#if selectItem.createUuid !== $charUUID}
            <div class="newphone__project_button auction" on:keypress={() => {}} on:click={() => isBet = true}>{translateText('player2', 'Предложить бид')}</div>
        {/if}
    {:else}
        {#if isConfirmed === false}
            ...
            <div class="newphone__project_button auction" on:keypress={() => {}} on:click={() => isConfirmed = true}>{translateText('player2', 'Принять участие')}</div>
        {:else}
            ...
            <div class="newphone__project_button auction mb-6" on:keypress={() => {}} on:click={onBet}>{translateText('player2', 'Я уверен')}</div>
        ...
        {/if}
    {/if}
    <div class="green box-center mt-5" on:keypress={() => {}} on:click={onBack}>{translateText('player2', 'Назад')}</div>
</div>