...
<div in:fade class="box-100">
    ...
    <div class="newphone__ads_info small">
        <textarea class="newphone__ads_textarea" bind:value={text} maxlength={maxSymbol} on:focus={onInputFocus} on:blur={onInputBlur} placeholder="A field for entering an ad ..." />
        <div class="newphone__ads_capture">
            {#if !cameraLink}
                <div class="newphone__ads_gray" on:keypress={() => {}} on:click={updateCameraToggled}>{translateText('player2', 'Добавить фото+')}</div>
            {:else}
                {#if cameraLink !== true}
                <div class="newphone__ads_addimg" style="background-image: url({cameraLink})"></div>
                {/if}
                <div class="box-column">
                    <span class="phoneicons-reload" on:keypress={() => {}} on:click={updateCameraToggled}></span>
                    <span class="phoneicons-trash" on:keypress={() => {}} on:click={() => cameraLink = false}></span>
                </div>
            {/if}
        </div>
    </div>
    ...
    <div class="newphone__project_button" on:keypress={() => {}} on:click={onSend}>{translateText('player2', 'Подать объявление')}</div>
    <div class="red box-center" on:keypress={() => {}} on:click={onBack}>{translateText('player2', 'Назад')}</div>
</div>