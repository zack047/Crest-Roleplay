...
{:else}
    <div in:fade class="box-100">
        <div class="newphone__project_button" on:keypress={() => {}} on:click={()=> onSelectedView ("Add")}>{translateText('player2', 'Подать объявление')}</div>
        {#if newsList && typeof newsList === "object" && newsList.length > 0}
            <div class="box-center box-between newphone__project_padding20">
                ...
                {:else}
                    <div class="newphone__button_filter box-center red__background" on:keypress={() => {}} on:click={openFilter}>
                            ...
                            <div class="box-between">
                                ...
                                <div class="box-flex">
                                    <div class="newphone__ads_circle">
                                        <span class="phoneicons-call" on:keypress={() => {}} on:click={() => onCall (item.number)}></span>
                                    </div>
                                    <div class="newphone__ads_circle">
                                        <span class="phoneicons-chat" on:keypress={() => {}} on:click={() => onMessage (item.number)}></span>
                                    </div>
                                </div>
                            </div>
                        </div>
