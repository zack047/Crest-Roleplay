...
<div class="newphone__rent_list" in:fade>
    ...
    {#each ordersList as order}
        <div class="newphone__rent_none hover" on:keypress={() => {}} on:click={() => onSelectItem(order.name)}>
    ...
    {/each}
    <div class="violet box-center m-top10" on:keypress={() => {}} on:click={() => onSelectedViewBusiness ("Menu")}>{translateText('player2', 'Назад')}</div>
</div>