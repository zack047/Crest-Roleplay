...
<div class="newphone__rent_list" in:fade>
    {#if propertyList && typeof propertyList === "object" && propertyList.length > 0}
        {#each propertyList as item}
            <div class="newphone__rent_none hover" on:keypress={() => {}} on:click={() => onSelectItem(item)}>
                <div class="box-column">
                    <div class="box-flex">
                        <div class="violet">{item.type === 0 ? "House" : "Business"}</div>
                        {#if item.isowner}
                            <div class="newphone__rent_status">{item.isOwner ? "Private" : "Squeezed"}</div>
                        {/if}
                    </div>
