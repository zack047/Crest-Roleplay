...
{#if selectedFurniture === null}
    <div class="newphone__rent_list">
        {#if houseFurnitures && typeof houseFurnitures === "object" && houseFurnitures.length > 0}
            ...
            <div class="newphone__rent_none hover" on:keypress={() => {}} on:click={() => onSelectedFurniture (index)}>
        ...
        {:else}
            ...
            <div class="newphone__project_button property" on:keypress={() => {}} on:click={setPoint}>{translateText('player2', 'Как добраться')}?</div>
            <div class="newphone__project_button property" on:keypress={() => {}} on:click={()=> currentPage.set("taxi")}>{translateText('player2', 'Вызвать такси')}</div>
        {/if}
        <div class="violet box-center m-top10" on:keypress={() => {}} on:click={() => onSelectedViewHouse ("Menu")}>{translateText('player2', 'Назад')}</div>
    </div>
{:else}
    <div class="newphone__rent_list">
        ...
        <div class="newphone__project_button" on:keypress={() => {}} on:click={() => onFurnitureBuy (1)} class:auction={!houseFurnitures [selectedFurniture].IsSet} class:property={houseFurnitures [selectedFurniture].IsSet}>{!houseFurnitures [selectedFurniture].IsSet ? translateText('player2', 'Установить') : translateText('player2', 'Убрать')}</div>
        <div class="newphone__project_button" on:keypress={() => {}} on:click={() => onFurnitureBuy (0)}>{translateText('player2', 'Продать')}</div>
        <div class="violet box-center m-top10" on:keypress={() => {}} on:click={() => onSelectedFurniture (null)}>{translateText('player2', 'Назад')}</div>
    </div>
{/if}