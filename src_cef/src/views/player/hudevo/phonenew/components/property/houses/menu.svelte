...
<div class="newphone__rent_list">
    ...
    {#if menuAccess.includes ("lock")}
        <div class="newphone__project_button property auction" on:keypress={() => {}} on:click={() => onAction ("leavehouse")}>{translateText('player2', 'Открыть/закрыть')}</div>
    {/if}
    {#if menuAccess.includes ("leavehouse")}
        <div class="newphone__project_button property" on:keypress={() => {}} on:click={() => onAction ("leavehouse")}>{translateText('player2', 'Выселиться')}</div>
    {/if}
    {#if menuAccess.includes ("removeall")}
        <div class="newphone__project_button property" on:keypress={() => {}} on:click={() => onAction ("removeall")}>{translateText('player2', 'Выгнать посетителей')}</div>
    {/if}
    {#if menuAccess.includes ("roommates") && residentsCount > 0}
        <div class="newphone__project_button property" on:keypress={() => {}} on:click={() => onSelectedViewHouse("Residents")}>{translateText('player2', 'Жильцы')}</div>
    {/if}
    {#if menuAccess.includes ("upgrades")}
        <div class="newphone__project_button property" on:keypress={() => {}} on:click={() => onSelectedViewHouse("Upgrade")}>{translateText('player2', 'Улучшения дома')}</div>
    {/if}
    {#if menuAccess.includes ("furniture")}
        <div class="newphone__project_button property" on:keypress={() => {}} on:click={() => onSelectedViewHouse("Furniture")}>{translateText('player2', 'Мебель')}</div>
    {/if}
    <div class="newphone__project_button property" on:keypress={() => {}} on:click={() => executeClient ("gps.pointDefault", "house")}>{translateText('player2', 'Показать на карте')}</div>
    {#if menuAccess.includes ("inPark")}
        <div class="newphone__project_button property"  on:keypress={() => {}} on:click={onOpenPark}>{translateText('player2', 'Выбор парковки')}</div>
    {/if}
    {#if menuAccess.includes ("sell") && houseData && Object.values (houseData) && Object.values (houseData).length}
        <div class="newphone__project_button"  on:keypress={() => {}} on:click={() => onAction ("sell")}>{translateText('player2', 'Продать за')} ${format("money", houseData.sellPrice)}</div>
    {/if}
    <div class="violet box-center m-top10" on:keypress={() => {}} on:click={() => onSelectedView ()}>{translateText('player2', 'Назад')}</div>
</div>