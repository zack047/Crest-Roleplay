<script>
    ...
    const onSelect = () => {
        ...
        if (product.minPrice > value || value > product.maxPrice ) {
            window.notificationAdd(4, 9, `${translateText('player2', 'Наценка не может быть меньше')} ${product.minPrice}${getPrefix (selectedProductName, bizType)} And more than ${product.maxPrice}${getPrefix (selectedProductName, bizType)}`, 3000);
            return;
        }
...
<div class="newphone__rent_list" in:fade>
    ...
    <input type="text" class="newphone__ads_input mb-0" placeholder="Enter the margin" bind:value={price} on:focus={onInputFocus} on:blur={onInputBlur} />
    <div class="newphone__project_button auction" on:keypress={() => {}} on:click={onSelect}>{translateText('player2', 'Обновить')}</div>
    <div class="violet box-center m-top10" on:keypress={() => {}} on:click={onSelectBack}>{translateText('player2', 'Назад')}</div>
</div>