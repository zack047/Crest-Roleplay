...
<div class="newphone__rent_list" in:fade>
    ...
    {#if !["Лотерейный билет", "Расходники"].includes(product.name)}
        <div class="newphone__project_button auction mb-0" on:keypress={() => {}} on:click={onChangePrice}>{translateText('player2', 'Изменить наценку')}</div>
    {/if}

    {#if !product.isOrder && product.maxCount > product.count}
        <div class="newphone__project_button property" on:keypress={() => {}} on:click={onOrderProduct}>{translateText('player2', 'Заказать')}</div>
    {/if}

    {#if product.isOrder && product.uidOrder}
        <div class="newphone__project_button property" on:keypress={() => {}} on:click={cancelOrder}>{translateText('player2', 'Отменить заказ')}</div>
    {/if}
