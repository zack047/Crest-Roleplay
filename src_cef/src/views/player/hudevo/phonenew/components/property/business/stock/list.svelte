...
<div class="newphone__rent_list" in:fade>
    ...
    {#if stats.whCount < stats.whMaxCount && stats.whPriceMaxCount > 0}
        <div class="newphone__project_button property" on:keypress={() => {}} on:click={onMaxProducts}>{translateText('player2', 'Заказать все за')} ${format("money", stats.whPriceMaxCount)}</div>
    {/if}
    {#each productsList as product}
        <div class="newphone__rent_none hover" on:keypress={() => {}} on:click={() => onSelectItem(product.name)}>
    ...
    {/each}
    <div class="violet box-center mt-15" on:keypress={() => {}} on:click={() => onSelectedViewBusiness ("Menu")}>{translateText('player2', 'Назад')}</div>
</div>