<script>
    ...
    const onSetBet = () => {
        if (!isBet) return;
            ...
            value = $charMoney;
            window.notificationAdd(4, 9, `You don't have that much money!`, 3000);
            return;
        } else if (Number (value) < 50) {            
            value = 50;
            window.notificationAdd(4, 9, `The minimum rate is $${format("money", 50)}`, 3000);
            return;
        } else if (Number (value) > Number (betMax)) {            
            value = betMax;
            window.notificationAdd(4, 9, `The maximum bet on this table is $${format("money", betMax)}`, 3000);
            return;
        }
...
<div id="casino-main">
    ...
    <div class="box-KeyInfo">
        ...
        {translateText('casino', 'Show/Hide cursor')}
    </div>
    <div class="bet-block">
        <div class="icon-dollar"/>
        <div class="info-area">
            <div class="gray-color">{translateText('casino', 'Balance')}:</div>
            <div class="balance-money">${format("money", $charMoney)}</div> 
        </div>
        {#if betWin > 0}
        <div class="e-win info-area">                        
            <div class="gray-color">{translateText('casino', 'Winning')}:</div>
        ...
        <input placeholder="Quantity for bid" maxLength="6" class="e-input" bind:value={value} on:input={(event) => onHandleInput (event.target.value)} disabled={!isBet} />
