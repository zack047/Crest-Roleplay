...
<div class="circle">
    <div class="circle__close" use:initCircle on:mouseenter={() => OnHovered ('Назад', true)} on:mouseleave={() => OnHovered ('Назад')} on:keypress={() => {}} on:click={() => onCircleClick ("back")}>
        {drawname}
    </div>
    <div class="center">
        {#each popupData as data, index}
        <li on:keypress={() => {}} on:click={() => onCircleClick (data.func, index)} on:mouseenter={() => OnHovered (data.name)} on:mouseleave={() => OnHovered ("Назад")} class="contents child{ontest (index, popupData.length)}">
