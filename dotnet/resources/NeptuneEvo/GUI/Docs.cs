...
namespace NeptuneEvo.GUI
{
    ...
    class Docs : Script
    {
        ...
        {
            new DocsClass("acceptPass", "your passport", "passport"),
            new DocsClass("acceptLics", "their licenses", "licenses"),
            new DocsClass("acceptIdcard", "his ID-card", "ID card"),
            new DocsClass("acceptCertificate", "his certificate", "credential")
        };
        ...
        public static void DocsFunction(ExtPlayer player, ExtPlayer target, int docsEnum)
        {
            try
            {
               ...
               // Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.PlayerOfferShowDoc, player.Value, DocsArray[docsEnum].Name), 6000);
                EventSys.SendCoolMsg(player,"Offer", "Documents", $"{LangFunc.GetText(LangType.Ru, DataName.YouOfferShowDoc, DocsArray[docsEnum].SendFrom)}", "", 7000);
                EventSys.SendCoolMsg(target,"Offer", "Documents", $"{LangFunc.GetText(LangType.Ru, DataName.PlayerOfferShowDoc, player.Value, DocsArray[docsEnum].Name)}", "", 8000);
