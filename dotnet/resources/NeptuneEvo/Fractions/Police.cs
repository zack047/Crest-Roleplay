...
namespace NeptuneEvo.Fractions
{
    class Police : Script
    {
        ...
        public static Vector3 ExitPrisonPosition = new Vector3(1851.0327, 2585.9954, 45.672016);
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Repair of ground transport"), VehicleRepairPosition, 5F, 0.3F, 0, new Color(255, 255, 255));

                Ped ped = PedSystem.Repository.CreateQuest("s_m_y_cop_01", new Vector3(442.6866f, -981.88605f, 30.689486f), 89.897484f, title: "~y~NPC~w~ Officer Anthony\nDuty", colShapeEnums: ColShapeEnums.FracPolic);
                PedSecretaryId = ped.Value;

                PedSystem.Repository.CreateQuest("s_m_m_fibsec_01", new Vector3(480.34195, -996.7015, 30.690102), 88.88336f, title: "~y~NPC~w~ Agent Robert\nWarehouse", colShapeEnums: ColShapeEnums.FracPolic);
                PedSystem.Repository.CreateQuest("s_f_y_cop_01", new Vector3(442.5702, -978.62854, 30.689486), 100.0675f, title: "~y~NPC~w~ Officer Fox\nTo call an employee", colShapeEnums: ColShapeEnums.CallPoliceMember);
            }
            catch (Exception e)
            {
        ...
        public static string OnCallPolice(ExtPlayer player, string reason)
        {
            try
            {
                ...
                if (characterData.Unmute > 0)
                {
                    ...
                    return "Unfortunately, we cannot accept your challenge :(";
                }
        ...
        public static void acceptCall(ExtPlayer player, int id)
        {
            try
            {
                ...
                else if (PoliceHouseCalls.ContainsKey(id))
                {
                    ...
                    if (Configs.IsFractionPolic(fracId))
                    {
                        Manager.sendFractionMessage(fracId, $"{player.Name.Replace('_', ' ')} took a call from home №{id}.");
                        Manager.sendFractionMessage(fracId, "!{#F08080}[F] " + $"{player.Name.Replace('_', ' ')} took a call from home №{id}", true);
                        player.AddTableScore(TableTaskId.Item5);
                    }
                }
                else if (PoliceCarsCalls.ContainsKey(id))
                {
                    ...
                    if (Configs.IsFractionPolic(fracId))
                    {
                        Manager.sendFractionMessage(fracId, $"{player.Name.Replace('_', ' ')} took a car alarm call №{id}.");
                        Manager.sendFractionMessage(fracId, "!{#F08080}[F] " + $"{player.Name.Replace('_', ' ')} took a car alarm call №{id}", true);
                        player.AddTableScore(TableTaskId.Item5);
        ...
        public static void clearWantedLvl(ExtPlayer sender, params object[] arguments)
        {
            try
            {
                ...
                if (vehiclenumb.Equals("null"))
                {
                    ...
                    Fractions.Table.Logs.Repository.AddLogs(sender, FractionLogsType.Su, $"The wanted list was lifted from the {target} ({tauuid})");
                ...
                else
                {
                    if (WantedVehicles.ContainsKey(vehiclenumb))
                    {
                        ...
                        Fractions.Table.Logs.Repository.AddLogs(sender, FractionLogsType.Su, $"Unsearched the vehicle ({vehiclenumb})");
                    }
                    else
                    {
        ...
        public static void checkPerson(ExtPlayer player, params object[] arguments)
        {
            try
            {
                ...
                try
                {
                    ...
                    fraction_name = fraction_name == String.Empty ? "Not available" : fraction_name;

                    string numberInfo = targetCharacterData.Sim == -1 ? "Not available" : targetCharacterData.Sim.ToString();
                ...
                catch
                {
                    ...
                    Trigger.SetTask(async () =>
                    {
                        try
                        {
                            ...
                            var fraction_name = "Not available";
                            ...
                            var numberInfo = sim == -1 ? "Not available" : sim.ToString();
        ...
        public static void PlayerSoloArrest(ExtPlayer player)
        {
            try
            {
                ...
                Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.Arrest, $"{player.Name} ({characterData.UUID}) surrendered red-handed and was taken to the detention center for {minutes} minutes.");
            }
            catch (Exception e)
            {
        ...
        public static void OpenSpecialPoliceMenu(ExtPlayer player)
        {
            try
            {
                ...
                if (!is_warg)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Emergency mode is not enabled", 3000);
                    return;
                }
