...
namespace NeptuneEvo.Fractions.Models
{
    public class FractionData : TableData
    {
        ...
        public void UpdateLabel()
        {
            ...
            {
                if (Drugs > 0) text += $"Drugs: ~r~{Drugs}/10000\n";
                if (Materials > 0) text += $"Materials: ~r~{Materials}/{MaxMats}\n";
                if (MedKits > 0) text += $"First aid kits: ~r~{MedKits}";
            }

            if (HaveSpecialStock == true) text += $"\n\nUnprocessed ore:\n~w~Fossil coal ~r~{CoalOre} ~w~units of iron ore ~r~{IronOre} ~w~unit/sulfur ore ~r~{SulfurOre} ~w~ед.\nGemstones ~r~{PreciousOre} ~w~unit.";
