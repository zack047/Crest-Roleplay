...
namespace NeptuneEvo.Fractions.LSNews.LiveStream
{
    public class LiveStream
    {
        ...
        public void Start()
        {
            Manager.sendFractionMessage(LsNewsFracid, $"FF8C00 [F] {_owner.Name} ({_owner.Id}) started the live broadcast.");
        }

        public void Stop()
        {
            Manager.sendFractionMessage(LsNewsFracid, $"FF8C00 [F] {_owner.Name} ({_owner.Id}) finished the live broadcast.");
        }
        ...
        private void SendMessageToListeners(IEnumerable<ExtPlayer> listeners, string msg, bool isOwner)
        {
            ...
            {
                string prefix = isOwner ? "#FAFAD2 [Weazel News] Presenter: " : " #FAFAD2 [Weazel News] Guest: ";
