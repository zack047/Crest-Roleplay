    ...
    class CarDelivery : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                foreach (KeyValuePair<int, Vector3> pos in MafiaStartDelivery)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel("~w~To take a delivery mission", pos.Value, 5f, 0.4f, 0, new Color(255, 255, 255));
                }
                #endregion
        ...
        public static void InPoliceDropDelivery(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You arrested the car", 3000);
                }
            }
            catch (Exception e)
            {
        ...
        public static void InGangEndDelivery(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Try a little later", 3000);
                        return;
                    }
                    VehicleStreaming.DeleteVehicle(vehicleDelivery);
                    ...
                    Trigger.SendChatMessage(player, "Surrendering a car: !{#00FF00}" + halfof + "$ ~w~were sent to the gang's fund and the same amount to your pocket. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()}" + ")");
        ...
        public static void InMafiaEndDelivery(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    fractionData.Money += halfof;
                    GameLog.Money($"server", $"frac({fractionData.Id})", halfof, "dropCar");
                    Trigger.SendChatMessage(player, "Surrendering a van: !{#00FF00}" + halfof + "$ ~w~were sent to the gang's common fund and the same amount to your pocket. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()})");
                    player.AddTableScore(TableTaskId.Item27);
                }
            }
            catch (Exception e)
                    ...
                    switch (vehicleLocalData.Access)
                    {
                        case VehicleAccess.DeliveryGang:
                            ...
                            else
                            {
                                ...
                                Trigger.SendChatMessage(player, "If you suddenly hit a delivery point, then write in the chat /mypoint");
                                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Take the car to the point marked on the map", 3000);
                            }
                            return;
            ...
            try
            {
                ...
                switch (id)
                {
                    ...
                    case 1:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Try again later", 3000);
                            return;
                        }
                        if (sessionData.DeliveryData.Vehicle != null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You must first deliver the previous machine", 3000);
                            return;
                        }
                        ...
                        Trigger.SendChatMessage(player, "If you suddenly hit the coordinates of the car, then write in a chat /mypoint");
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "You have the coordinates of the car. Get in it and take it to the place that is marked in the car's GPS.", 3000);
                        player.AddTableScore(TableTaskId.Item27);
                        return;
                    default:
                        // Not supposed to end up here. 
        ...
        public static void Event_mafiaMission(ExtPlayer player, int id)
        {
            try
            {
                ...
                if (sessionData.DeliveryData.Vehicle != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have to deliver the previous car first.", 3000);
                    return;
                }
                Random rnd = new Random();
                ...
                NextDelivery[fracId] = DateTime.Now.AddMinutes(5);
                Trigger.SendChatMessage(player, "If you suddenly hit the coordinates of the car, then write in a chat /mypoint");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You got the van with {text} for transportation. Take it to the location shown on the map", 3000);
                player.AddTableScore(TableTaskId.Item27);
                Trigger.ClientEvent(player, "createWaypoint", MafiaEndDelivery[end].X, MafiaEndDelivery[end].Y);
