...
namespace NeptuneEvo.Fractions
{
    class Ticket : Script
    {
        ...
        public static async void OnResourceStart()
        {
            ...
            //
            PedSystem.Repository.CreateQuest("s_m_m_ciasec_01", PedPos, PedRot, questName: QuestName, title: "~y~NPC~w~ Robert\nPolice officer", colShapeEnums: ColShapeEnums.ImpoundLotPed, pedBlipData: new PedBlipData
            {
                BlipId = 783,
                ...
                if (vehicleTicket.Type == VehicleTicketType.Player)
                {
                    ...
                    if (vehiclesNumber.Contains(vehicleTicket.VehNumber))
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter,
                            $"You bought the car back from the impound lot.", 3000);
