    ...
    class Ems : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Open storage"), new Vector3(emsCheckpoints[3].X, emsCheckpoints[3].Y, emsCheckpoints[3].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));

                CustomColShape.CreateCylinderColShape(emsCheckpoints[4], 1, 2, 0, ColShapeEnums.FractionEms, 2); // duty change
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Click\n~r~'Interaction'"), new Vector3(emsCheckpoints[4].X, emsCheckpoints[4].Y, emsCheckpoints[4].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));



                CustomColShape.CreateCylinderColShape(emsCheckpoints[6], 1, 2, 0, ColShapeEnums.FractionEms, 4); // tattoo delete
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Tattoo removal"), new Vector3(emsCheckpoints[6].X, emsCheckpoints[6].Y, emsCheckpoints[6].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~To the hospital."), new Vector3(emsCheckpoints[7].X, emsCheckpoints[7].Y, emsCheckpoints[7].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));

                CustomColShape.CreateCylinderColShape(emsCheckpoints[8], 1, 2, 0, ColShapeEnums.FractionEms, 6); // to roof
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~On the roof"), new Vector3(emsCheckpoints[8].X, emsCheckpoints[8].Y, emsCheckpoints[8].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                PedSystem.Repository.CreateQuest("s_m_m_scientist_01", new Vector3(315.63, -585.912, 43.26), 75.82f, title: "~y~NPC~w~ Emmanuel\nPhysician on duty", colShapeEnums: ColShapeEnums.FracEms);
                PedSystem.Repository.CreateQuest("s_m_y_autopsy_01", new Vector3(318.3019, -590.71857, 43.266674), 164.38774f, title: "~y~NPC~w~ Michael\nVeterinarian", colShapeEnums: ColShapeEnums.FracEmsVeterinarian);
                PedSystem.Repository.CreateQuest("s_m_m_doctor_01", new Vector3(315.7822, -588.0856, 43.261223), 107.24092f, title: "~y~NPC~w~ Stanislav, the medical brother\nTo call an employee", colShapeEnums: ColShapeEnums.CallEmsMember);
                PedSystem.Repository.CreateQuest("a_f_y_business_04", new Vector3(-1289.9895, -572.3277, 30.572992), -45.4007f, title: "~y~NPC~w~ Sara Karimova\nTo call an employee", colShapeEnums: ColShapeEnums.CallGovMember);
            }
            catch (Exception e)
            {
        ...
        public static string OnCallEms(ExtPlayer player, bool death = false)
        {
            try
            {
                ...
                if (EMSCalls.ContainsKey(player)) 
                    return "You've already made the call";
                ...
                else
                {
                    if (characterData.InsideHouseID != -1 || characterData.InsideGarageID != -1 || characterData.InsideOrganizationID != -1) 
                        return "You can't call the paramedics here";
                }
        ...
        public static void DeathConfirm(ExtPlayer player)
        {
            try
            {
                ...
                EventSys.SendCoolMsg(player, "Information", "You are Unconcious!", $"If no one cures you within 1 minute, you will be taken to the hospital", "", 30000);
        ...
        public static void ReviveFunc(ExtPlayer player, bool reviveonplace = false)
        {
            try
            {
                ...
                if (!reviveonplace)
                {
                    ...
                    {
                        spawnPos = Admin.DemorganPositions[Main.rnd.Next(25)] + new Vector3(0, 0, 1.12);
                        player.Dimension = 2;
                    }
        ...
        public static void Perform(ExtPlayer player)
        {
           ...
           EventSys.SendCoolMsg(player,"Hospital", "Treatment", $"{LangFunc.GetText(LangType.Ru, DataName.HealStarted)}", "", 5000); 
           BattlePass.Repository.UpdateReward(player, 15);
            BattlePass.Repository.UpdateReward(player, 98);
            ...
            try
            {
                ...
                switch (listItem)
                {
                    case "takemed":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There are no first aid kits left in the warehouse", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You took the first aid kit. You have {Chars.Repository.getCountItem($"char_{player.GetUUID()}", ItemId.HealthKit)} pieces", 3000);
                    ...
                    case "putmed":
                        ...
                        if (ItemStruct == null)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have first aid kits", 3000);
                            return;
                        }
                        Chars.Repository.RemoveIndex(player, ItemStruct.Location, ItemStruct.Index, 1);
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You put down the first aid kit. You have {Chars.Repository.getCountItem($"char_{player.GetUUID()}", ItemId.HealthKit)} pieces", 3000);
                        fractionData.MedKits++;
                        fractionData.UpdateLabel();
                        GameLog.Stock(memberFractionData.Id, player.GetUUID(), player.Name, "medkit", 1, "in");
                        Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeMedkits, $"I put a first aid kit in storage");
                        break;
                    case "tazer":
                        ...
                        Fractions.Table.Logs.Repository.AddLogs(player, FractionLogsType.TakeSpecial, $"Created by StunGun ({serial})");
        ...
        private static string[] TattooZonesNames = new string[6] { "torso", "heads", "left hand", "right hand", "left leg", "right leg" };
        private static void callback_tattoodelete(ExtPlayer player, object listItem)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have no tattoos in this area", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You brought the tattoos down to " + TattooZonesNames[Convert.ToInt32(zone)], 3000);
            }
            catch (Exception e)
            {
