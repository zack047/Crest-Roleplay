...
namespace NeptuneEvo.Fractions
{
    class AlcoFabrication : Script
    {
        ...
        {
            { (int) Models.Fractions.LCN, "Bahama Mamas West Club" },
            { (int) Models.Fractions.RUSSIAN, "Club Vanilla" },
            { (int) Models.Fractions.YAKUZA, "Club Tequilla" },
            { (int) Models.Fractions.ARMENIAN, "Club Diamond" },
        };
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(121, "Bahama Mamas West Club", new Vector3(-1388.761, -586.3921, 29.09945), 0, true));
                Main.CreateBlip(new Main.BlipData(121, "Club Vanilla", new Vector3(141.3792, -1281.576, 28.2172), 0, true));
                Main.CreateBlip(new Main.BlipData(121, "Club Tequilla", new Vector3(-564.5512, 275.6993, 81.98249), 0, true));
                //Main.CreateBlip(new Main.BlipData(121, "Клуб Diamond", new Vector3(945.4526, 16.871, 115.0442), 0, true));
                
                Main.CreateBlip(new Main.BlipData(136, "Café Stand Up", new Vector3(-439.601, 271.704, 82.015), 0, true));
                Main.CreateBlip(new Main.BlipData(136, "Chinese restaurant", new Vector3(-158.242, 303.829,98.822), 0, true));
                ...
                foreach (KeyValuePair<int, Vector3> pair in ExitAlcoShop)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~w~Exit", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
                #region Unloadpoints
                foreach (KeyValuePair<int, Vector3> pair in UnloadPoints)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel($"~w~Unload materials", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
                #region BuyPoints
                foreach (KeyValuePair<int, Vector3> pair in BuyPoints)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~w~Buy alcohol", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
            }
            catch (Exception e)
            ...
            try
            {
                ...
                switch (action)
                {
                    case 0: // buy
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Insufficient goods", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought {Chars.Repository.ItemsInfo[invItem].Name}", 3000);
                        return;
                    case 1: // take
                        if (alcoCounts[index] <= 0)
                        {
                        ...
                        ClubsStocks[fractionId].UpdateLabel();
                        //OpenBuyAlcoholMenu(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You took {Chars.Repository.ItemsInfo[invItem].Name}. In stock {alcoCounts[index] - 1}piece", 3000);
                        return;
                    case 2: // craft
                        if (alcoCounts[index] >= 80)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"In the warehouse maximum {Chars.Repository.ItemsInfo[invItem].Name}", 3000);
                            return;
                        }
                        ...
                        ClubsStocks[fractionId].UpdateLabel();
                        //OpenBuyAlcoholMenu(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have crafted {Chars.Repository.ItemsInfo[invItem].Name}. In stock {alcoCounts[index] + 1}piece", 3000);
                        return;
                    case 3: // setprice
                        Trigger.ClientEvent(player, "openInput", "Set a price", "Enter the price for alcohol in percentages", 3, "club_setprice");
                        return;
                    default:
                        // Not supposed to end up here. 
                        break;
        ...
        public static void SetAlcoholPrice(ExtPlayer player, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Set the price from 50% to 150%", 3000);
                    return;
                }
        ...
        internal class Stock
        {
            ...
            {
                Label = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel($"~w~Materials: ~r~{mats}\n~w~Alcohol: ~r~{a1 + a2 + a3}", pos, 30f, 0.3f, 0, new Color());
            ...
            {
                Label.Text = $"~w~Materials: ~r~{Materials}\n~w~Alcohol: ~r~{Alco1 + Alco2 + Alco3}";
            }
        }
    }
}
