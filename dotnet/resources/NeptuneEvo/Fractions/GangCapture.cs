    ...
    class GangsCapture : Script
    {
        ...
        public static void CMD_startCapture(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have been restricted from accessing the capture and bizwar systems. The reason is: {characterData.BanCrimeReason}.", 5000);
                    return;
                }
                ...
                Trigger.SendToAdmins(1, $"{ChatColors.Red}[A] {player.Name} ({player.Value}) From {Manager.GetName(warData.AttackingId)} provoked a fight over territory {Manager.GetName(warData.ProtectingId)}.");
                GameLog.FracLog(fracId, characterData.UUID, -1, player.Name, "-1", $"capture({region.ID}, {Manager.FractionNames[region.GangOwner]})");

                Manager.SendCoolFractionMsg(warData.ProtectingId, "Capt", "you were attacked!", LangFunc.GetText(LangType.Ru, DataName.CaptureAlertDefend, Manager.GetName(warData.AttackingId)), "", 10000);
                Manager.SendCoolFractionMsg(warData.AttackingId, "Capt", "Attack", LangFunc.GetText(LangType.Ru, DataName.CaptureAlertAttack), "", 10000);
        ...
        public static void CMD_tpcapture(ExtPlayer player)
        {
            try
            {
                ...
                if (characterData.IsBannedCrime == true)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have been restricted from accessing the capture and bizwar systems. Reason: {characterData.BanCrimeReason}.", 5000);
                    return;
                }
                ...
                if (war != null && (fracId == war.ProtectingId || fracId == war.AttackingId))
                    ...
                    if (fracId != war.ProtectingId)
                    {
                        ...
                        EventSys.SendCoolMsg(player,"Capt", "Teleport", LangFunc.GetText(LangType.Ru, DataName.YouWasTpCenterClaimRegion), "", 5000);   
                    ...
                    else
                    {
                        ...
                        if (nearestCaptureCoord != null)
                        {
                            ...
                            sessionData.UsedTPOnCaptureOrBizwar = 1;
                            EventSys.SendCoolMsg(player,"Capt", "Teleport", LangFunc.GetText(LangType.Ru, DataName.YouWasTpNearClaimRegion), "", 5000);   
