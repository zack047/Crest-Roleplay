    ...
    class Donations : Script
    {
        ...
        private static void CheckPromoDonate(string login, ulong reds, ExtPlayer player)
        {
            try
            {
                ...
                if (reds >= 1)
                {
                    ...
                    {
                        UpdateData.RedBucks(target, (int)reds, msg: "Refill for referral");
                    ...
                    foreach (DataRow Row in result.Rows)
                    {
                        ...
                        {                       
                            ...
                            if (!DonatePack.IsDonate(player, name.ToLower(), (int)reds))
                            {
                                ...
                                if (accountData == null)
                                {
                                    GameLog.AccountLog(name, "-", "-", "-", $"Replenishment (+{reds} RedBucks)");
                                    using MySqlCommand cmd = new MySqlCommand
                                    {
                                ...
                                {
                                    UpdateData.RedBucks(player, (int)reds, msg: "Replenishment");
        ...
        public void GiftDonate(ExtPlayer player, string login, int sum)
        {
            try
            {
                ...
                UpdateData.RedBucks(player, -sum, msg: $"Gift {login}");
                ...
                if (targetAccountData == null)
                {
                    UpdateData.RedBucks(target, sum, msg: $"A gift from {accountData.Login}");
                    ...
                    Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"You received a gift: {sum} RedBucks from {player.Name.Replace('_', ' ')} ({player.Value})!", 5000);
                }
                else
                {
