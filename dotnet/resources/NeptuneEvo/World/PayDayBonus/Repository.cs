        ...
        public static void Bonus()
        {
            ...
            if (players.Count > 0)
            {
                ...
                foreach (var foreachPlayer in winnersPlayer)
                {
                    ...
                    UpdateData.RedBucks(foreachPlayer, Main.DonateSettings.HappyHoursRB, msg: "HAPPY HOURS");
                    Trigger.ClientEvent(foreachPlayer, "hud.info", "Luck is on your side!", $"Congratulations on winning the drawing! {Main.DonateSettings.HappyHoursRB} RedBucks have already been credited to your account.", "HAPPY HOURS", "https://prdx.to/cdn/cloud/img/time.png");

                    winersName += $" {foreachSessionData.Name}";
                }
            
                NAPI.Chat.SendChatMessageToAll($"~o~[HAPPY HOURS] Drawing {Main.DonateSettings.HappyHoursRB} RedBucks is held every hour only among active players.");
                NAPI.Chat.SendChatMessageToAll($"~o~[HAPPY HOURS] The winners of this hour are: {winersName}. They get by {Main.DonateSettings.HappyHoursRB}, congratulations!");   
            }
        }
