        ...
        public static void Open(ExtPlayer player, ushort objectId, ushort mapId, string mapName, Vector3 position, float range, WarType type, ushort attackingId, ushort protectingId)
        {
            try
            {
                ...
                if (type == WarType.Gangs)
                    title = $"Zone attack {mapName}";
                if (type == WarType.Mafia)
                    title = $"Attack on the bizz {mapName}";
                if (type == WarType.OrgHouse)
                    title = $"Home invasion {mapName}";
        ...
        public static ushort War(ExtPlayer player, Players.Models.WarData warData, sbyte typeBattle, sbyte composition, sbyte weaponsCategory, sbyte day = 0, sbyte hour = 0, sbyte min = 0, bool isCheckTime = true, bool isWarInterface = true)
        {
            ...
            try
            {
                ...
                if (isCheckTime)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Wrong time.", 6000);
                        //if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                        return 0;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The arrow can be scored in at least two hours", 3000);
                        //if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                        return 0;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The arrow can be scored in at least 5 minutes", 3000);
                        //if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                        return 0;
                    }
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This area is already in the process of being taken over.", 8000);
                    if (isWarInterface) Trigger.ClientEvent(player, "client.closeWar");
                    return 0;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You scored an arrow.", 15000);
        ...
        public static void InWarZone(ExtPlayer player, WarType type, int index)
        {
            try
            {
                ...
                else
                {
                    ...
                    frameList.Header = "Selecting a War Zone";
                    ...
                    foreach (var war in wars)
                    {
                        ...
                        if (war.Type == WarType.Gangs)
                            title = "The war for the zone";
                        else if (war.Type == WarType.Mafia)
                            title = "War for business";
        ...
        private static void UpdateWar(ushort warId)
        {
            try
            {
                    ...
                    if (war.Time <= time)
                    {
                        ...
                        else
                        {
                            if (war.GripType == WarGripType.RetentionTerritory)
                            {
                                ...
                                Notification(war.Id, "The enemy team can enter the dome.");
                            }
                            else
                            {
                ...
                else if (war.Status == WarStatus.Protection)
                {
                        ...
                        war.Status = WarStatus.War;
                        war.Counting = WarCounting;
                        Notification(war.Id, "The attack team can enter the dome.");
            ...
            try
            {
                ...
                switch (war.Type)
                {
                    case WarType.OrgWarZone:
                        if (draw)
                        {
                            Fractions.Manager.SendCoolOrganizationMsg(war.ProtectingId,"War", "Tie", "You played to a draw.", "", 10000);   
                            Fractions.Manager.SendCoolOrganizationMsg(war.AttackingId,"War", "Tie", "You played to a draw.", "", 10000); 
                        ...
                        {
                            Fractions.Manager.SendCoolOrganizationMsg(war.ProtectingId,"War", "Defeat", "You lost the war and lost territory :(", "", 10000);   
                            Fractions.Manager.SendCoolOrganizationMsg(war.AttackingId,"War", "Victory", "You win! The territory goes under your control, congratulations!", "", 10000); 
                        ...
                        {
                            Fractions.Manager.SendCoolOrganizationMsg(war.ProtectingId,"War", "Victory", "You held the territory! Congratulations!", "", 10000);   
                            Fractions.Manager.SendCoolOrganizationMsg(war.AttackingId,"War", "Defeat", "You lose! Territory stays with the defending side.", "", 10000); 
                    ...
                    case WarType.Gangs:
                        if (victoryAttackers)
                        {
                            Fractions.Manager.SendCoolFractionMsg(war.ProtectingId,"Capt", "Defeat", LangFunc.GetText(LangType.Ru, DataName.CaptureLoseByDef), "", 10000);   
                            Fractions.Manager.SendCoolFractionMsg(war.AttackingId,"Capt", "Victory", LangFunc.GetText(LangType.Ru, DataName.CaptureWinByAttack), "", 10000); 
                        ...
                        {
                            Fractions.Manager.SendCoolFractionMsg(war.ProtectingId,"Capt", "Victory", LangFunc.GetText(LangType.Ru, DataName.CaptureWinByDef), "", 10000);   
                            Fractions.Manager.SendCoolFractionMsg(war.AttackingId,"Capt", "Defeat", LangFunc.GetText(LangType.Ru, DataName.CaptureLoseByAttack), "", 10000);   
