...
namespace NeptuneEvo
{
    ...
    {
        public static string Codename { get; } = "DeniAge";
        public static string Version { get; } = "v1.33.7";
        public static string Build { get; } = "#69";
        ...
        public static readonly string[] stringDefaultBlock = {
            "faggot", "Fag", "Fag", "faggot", "fag", "fag", "faggot", "faggot", "faggot", "faggot", "faggot", "faggot", "faggot", "faggots", "faggots", "faggot", "faggot", "faggot", "faggot", "faggot",
            "nigga", "n i g g a", "n i g g a", "n i g g a", "n.i.g.g.a", "n.i.g.g.a", "n.i.g.g.a"
        };
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                CreateBlip(new BlipData(93, "Beach parties", new Vector3(-1497.7688, -1484.525, 5.7608714), 43, true)); // todo LangFunc
                CreateBlip(new BlipData(766, "The Island", new Vector3(5024.207, -5122.8962, 3.701595), 43, true)); // todo LangFunc
                CreateBlip(new BlipData(93, "Beach parties", new Vector3(-1705.88, -970.7998, 8.648665), 43, true)); // todo LangFunc
                ...
                Timers.Start("savedb", 1000 * 60 * 2, () => Admin.SaveServer(), true);
            ...
            try
            {
                ...
                else if (characterData.DemorganTime > 0)
                {
                    if (sessionData.TimersData.ArrestTimer == null)
                    {
                        ...
                        NAPI.Entity.SetEntityPosition(player, Admin.DemorganPositions[rnd.Next(25)] + new Vector3(0, 0, 1.5));                       
            ...
            try
            {
                ...
                switch (callback)
                {
                    ...
                    case "streetrace":
                        ...
                        {
                            Trigger.ClientEvent(player, "openInput", "Street Race", LangFunc.GetText(LangType.Ru, DataName.InputBet0100k), 6, "streetrace");
                            return;
                        }
                    ...
                    case "sellcar":
                        ...
                        EventSys.SendCoolMsg(player, "Offer", "Purchase", $"{LangFunc.GetText(LangType.Ru, DataName.BuycarOffer, target.Name, vName, number, MoneySystem.Wallet.Format(limit))}", "", 5000);
                    ...
                    case "sell_tent":
                        ...
                        if (inventoryTentData.Value == Item.Count)
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, LangFunc.GetText(LangType.Ru, DataName.SellItemTent, "the item", limit), 10000);
                            return;
                        }
                    ...
                    case "player_ticketsum":
                        ...
                        Trigger.ClientEvent(player, "openInput", "Write a fine (reason)", "Reason", 50, "player_ticketreason");
                        break;
            ...
            try
            {
                ...
                switch (door.Split('_')[0])
                {
                    ...
                    case "ballas":
                        ...
                        break;
                    case "jail": // this allows only POLICE, FIB, GOV and SAHP to lock/unlock the prison doors
                        if (fractionId != (int)Fractions.Models.Fractions.POLICE && fractionId != (int)Fractions.Models.Fractions.FIB && fractionId != (int)Fractions.Models.Fractions.CITY && fractionId != (int)Fractions.Models.Fractions.SHERIFF) return;
                        else if (!player.IsFractionAccess(RankToAccess.DoorControl)) return;
                        break;
                    default:
                        return;
                        ...
                        case "BUY_CAR":
                            {
                                ...
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought {vehicleData.Model} ({number}) for {MoneySystem.Wallet.Format(price)}$ from {seller.Name}", 3000);
                                Notify.Send(seller, NotifyType.Success, NotifyPosition.BottomCenter, $"{player.Name} bought from you {vehicleData.Model} ({number}) for {MoneySystem.Wallet.Format(price)}$", 3000);
                                sellerSessionData.SellItemData = new SellItemData();
                                sessionData.SellItemData = new SellItemData();
                        ...
                        case "INVITED_FRAC":
                            {
                                try
                                {
                                    if (sessionData.InviteData.Fraction != -1)
                                    {
                                        ...
                                        EventSys.SendCoolMsg(player, "Fraction", "Welcome!", LangFunc.GetText(LangType.Ru, DataName.JoinFrac, Fractions.Manager.FractionNames[fractionId]), "", 8000);
                        ...
                        case "INVITED_ORG":
                            {
                                try
                                {
                                    if (sessionData.InviteData.Organization != -1)
                                    {
                                        ...
                                        EventSys.SendCoolMsg(player, "Organization", "Welcome!", LangFunc.GetText(LangType.Ru, DataName.JoinFrac, organizationData.Name), "", 8000);
        ...
        public Main()
        {
            try
            {
                NAPI.Task.Run(async () =>
                {
                    while (true)
                    {
                        ...
                        foreach (var player in NAPI.Pools.GetAllPlayers().Cast<ExtPlayer>())
                        {
                            ...
                            {
                                Trigger.SendToAdmins(1, $"~r~ Player {player.Name} ({player.Id}) {player.Address} is automatically unprotected and kicked off the server.");
                                player.Kick();
                            }
                        }
                    }
                ...
                NAPI.Server.SetCommandErrorMessage("The command was not found!");
                NAPI.Server.SetGlobalDefaultCommandMessages(true);
                //NAPI.Resource.StartResource("Teammate");//To reboot
        ...
        private static void OnlineReward(ExtPlayer player, byte hours, int redbuckses)
        {
            try
            {
                ...
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                UpdateData.RedBucks(player, redbuckses, msg: "Reward for the day online");
                ...
                EventSys.SendCoolMsg(player, "System", "Award for online", $"{LangFunc.GetText(LangType.Ru, DataName.BonusEverdayOnline, hours, redbuckses)}", "", 10000);
            ...
            try
            {
                ...
                if (AutoRestart) // Check AutoRestart
                {
                    if (now.Hour == 6)
                    {
                        if (now.Minute == 0)
                        {
                            NAPI.Chat.SendChatMessageToAll("!{#FF0000}[AUTO RESTART] Dear players, at 06:15 there will be an automatic restart of the server.");
                        }
                        else if (now.Minute == 5)
                        {
                            NAPI.Chat.SendChatMessageToAll("!{#FF0000}[AUTO RESTART] Dear players, we remind you that at 06:15 there will be an automatic restart of the server.");
                        }
                        ...
                        else if (now.Minute == 15 || now.Minute == 16)
                        {
                            ...
                            {
                                NAPI.Chat.SendChatMessageToAll("!{#FF0000}[AUTO RESTART] Dear players, now there will be an automatic restart of the server, the server will be available again in about a couple of minutes.");
                                Admin.stopServer("There is an automatic reboot, we will be happy to see you again in a couple of minutes! :)");
                            }
                        }
                    }
                }
                ...
                if (dayofweek == DayOfWeek.Wednesday || dayofweek == DayOfWeek.Friday || dayofweek == DayOfWeek.Sunday)
                {
                    if (now.Hour == 19 && now.Minute == 45)
                    {
                        ...
                        EventSys.SendPlayersToEvent("MATWAR", "War on materials!", $"At 8:00 p.m. the war for materials will begin! Fractions, prepare yourselves!", "", 15000);
                    }
                        ...
                        switch (foreachCharacterData.Time.TodayTime)
                        {
                            ...
                            case 180:
                                ...
                                if (!foreachCharacterData.Achievements[17])
                                {
                                    ...
                                    Notify.SendToKey(foreachPlayer, NotifyType.Alert, NotifyPosition.BottomCenter, $"In the donate menu in the Roulette section, a Free Daily Case is now available to you. You can open it for free once a day after 3 hours.", 30000, 21);
                                }
                                break;
                            case 240:
                        ...
                        if (pcdata.UsedLimit == 0 || pcdata.UsedTimes < pcdata.UsedLimit)
                        {
                            ...
                            if (pcdata.RewardVipLvl != 0)
                            {
                                ...
                                else if (accountData.VipLvl == pcdata.RewardVipLvl)
                                {
                                    ...
                                    GameLog.Money($"system", $"player({characterData.UUID})", 0, $"bonusVIP({accountData.VipLvl}lvl, {pcdata.RewardVipDays}d, down to {accountData.VipDate.ToString("s")})");
                                }
                                else
                                {
                                    uint days = 0;
                                    ...
                                    GameLog.Money($"system", $"player({characterData.UUID})", 0, $"bonusVIP({accountData.VipLvl}lvl, {days}d, down to {accountData.VipDate.ToString("s")})");
            ...
            try
            {
                            ...
                            if (pcdata.RewardLimit == 0 || pcdata.RewardReceived < pcdata.RewardLimit)
                            {
                                ...
                                if (pcdata.RewardVipLvl != 0)
                                {
                                    if (accountData.VipLvl == 0)
                                    {
                                        accountData.VipLvl = pcdata.RewardVipLvl;
                                        accountData.VipDate = DateTime.Now.AddDays(pcdata.RewardVipDays);
                                        GameLog.Money($"system", $"player({characterData.UUID})", 0, $"promoCodeVIP({accountData.VipLvl}lvl, {pcdata.RewardVipDays}d, down to {accountData.VipDate.ToString("s")})");
                                    }
                                    else if (accountData.VipLvl == pcdata.RewardVipLvl)
                                    {
                                        accountData.VipDate = accountData.VipDate.AddDays(pcdata.RewardVipDays);
                                    ...
                                    else
                                    {
                                        ...
                                        Trigger.SendChatMessage(player, $"Since you already had VIP status, instead of replacing it, you received extra credit {days} days of use!");
                                    }
                ...
                if (accountData.RefferalId != 0 && !accountData.RefPresentGet)
                {
                    if (characterData.LVL >= 3)
                    {
                        ...
                        UpdateData.RedBucks(player, 100, msg: "Gift for entering a ref code");
                        ...
                        Trigger.SetTask(async () =>
                        {
                            try
                            {
                                ...
                                if (targetAccountData != null)
                                {
                                    ...
                                    {
                                        successText += $" {Group.GroupNames[refferalData.VipId]} ({refferalData.VipDays} Days);";
                                    ...
                                    {
                                        successText += $" Game currency (+{MoneySystem.Wallet.Format(refferalData.Money)}$);";
                                    ...
                                    if (refferalData.RB > 0)
                                    {
                                        ...
                                        UpdateData.RedBucks(target, refferalData.RB, msg: "Referral Bonus.");
                                    }
                                ...
                                else
                                {
                                    ...
                                    if (refferalData.VipId > 0)
                                    {
                                        ...
                                        GameLog.Money($"server", $"player({refferalId})", 1, $"RefVIP({vipLvl}lvl, down to: {vipDate.ToString("s")})");
                                    }
                ...
                if (accountData.PromoCodes[0] == "noref" && accountData.RefferalId == 0 && !accountData.PresentGet && !accountData.RefPresentGet)
                    Trigger.SendChatMessage(player, "~r~You didn't specify a promo code when registering, but you can still activate any of the promo codes via your phone (K -> Gift).");
            }
            catch (Exception e)
            {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (PlayerNames.ContainsKey(winuuid)) // Если персонаж победителя еще существует
                            {
                                ...
                                NAPI.Chat.SendChatMessageToAll($"~g~[LOTTERY] {PlayerNames[winuuid]} hit the jackpot {MoneySystem.Wallet.Format(MoneySystem.Lottery.Price)}$ on the off chance {(MoneySystem.Lottery.LotteryBought.Where(p => p.Value == winuuid).Count() * 100.0 / MoneySystem.Lottery.LotteryBought.Count()).ToString("0.##")}%.");
                            ...
                            else NAPI.Chat.SendChatMessageToAll("~g~[LOTTERY] Due to a technical error, the drawing will be held in an hour.");
                        }
                    }
                    catch (Exception e)
                    {
        ...
        public void CMD_BUILD(ExtPlayer player)
        {
            ...
            {
                if (player == null) return;
                Trigger.SendChatMessage(player, $"Build !{{#f39c12}}{Full}!{{#FFF}} launched !{{#f39c12}}{StartDate}");
                Trigger.SendChatMessage(player, $"Compile date: !{{#f39c12}}{CompileDate}");
            }
            catch (Exception e)
            {
        ...
        public static List<ExtPlayer> GetPlayersInRadiusOfPosition(Vector3 position, float radius, uint dimension = 39999999)
        {
            try
            {
                //if (Thread.CurrentThread.Name != "Main") return new List<Player>(); // Left-flow protection, since I can't return a value from under NAPI.Task.Run.
        ...
        private static void callback_mayormenu(ExtPlayer player, object listItem) /// Никитос Чини 
        {
            try
            {
                ...
                {
                    case "take":
                        Trigger.ClientEvent(player, "openInput", "Get money from the treasury", "Quantity", 6, "mayor_take");
                        return;
                    case "put":
                        Trigger.ClientEvent(player, "openInput", "Put money in the treasury", "Quantity", 6, "mayor_put");
                        return;
                    case "fuelcontrol":
        ...
        public static void OpenFuelcontrolMenu(ExtPlayer player)
        {
            try
            {
                ...
                var fractionData = Fractions.Manager.GetFractionData((int)Fractions.Models.Fractions.CITY);
                if (fractionData != null)
                    frameList.List.Add(new ListData($"City Hall. Remains today.: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_city"));

                frameList.List.Add(new ListData(LangFunc.GetText(LangType.Ru, DataName.SetLimit), "set_city"));

                fractionData = Fractions.Manager.GetFractionData((int)Fractions.Models.Fractions.POLICE);
                if (fractionData != null)
                    frameList.List.Add(new ListData($"Police. Remains today: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_police"));

                frameList.List.Add(new ListData(LangFunc.GetText(LangType.Ru, DataName.SetLimit), "set_police"));

                fractionData = Fractions.Manager.GetFractionData((int)Fractions.Models.Fractions.EMS);
                if (fractionData != null)
                    frameList.List.Add(new ListData($"EMS. Remaining today: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_ems"));

                frameList.List.Add(new ListData(LangFunc.GetText(LangType.Ru, DataName.SetLimit), "set_ems"));

                fractionData = Fractions.Manager.GetFractionData((int)Fractions.Models.Fractions.FIB);
                if (fractionData != null)
                    frameList.List.Add(new ListData($"FIB. Remaining today: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_fib"));

                frameList.List.Add(new ListData(LangFunc.GetText(LangType.Ru, DataName.SetLimit), "set_fib"));

                fractionData = Fractions.Manager.GetFractionData((int)Fractions.Models.Fractions.ARMY);
                if (fractionData != null)
                    frameList.List.Add(new ListData($"Army. Remains today: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_army"));

                frameList.List.Add(new ListData(LangFunc.GetText(LangType.Ru, DataName.SetLimit), "set_army"));

                fractionData = Fractions.Manager.GetFractionData((int)Fractions.Models.Fractions.LSNEWS);
                if (fractionData != null)
                    frameList.List.Add(new ListData($"News. Remains today: {fractionData.FuelLeft}/{fractionData.FuelLimit}$", "info_news"));
