...
namespace NeptuneEvo.VehicleModel
{
    public class EliteAutoRoom : Script
    {
        ...
        public void Event_ResourceStart()
        {
            ...
            PedSystem.Repository.CreateQuest("s_m_m_movprem_01", NpcBuyPosition, NpcBuyRotation, title: "~y~NPC~w~ Luxury Transport Salesman", colShapeEnums: ColShapeEnums.EliteAutoRoom);
        }
        ...
        public static void Perform(ExtPlayer player)
        {
            try
            {
                ...
                var eliteVehiclesInfo = BusinessManager.BusProductsData
                    .Where(b => b.Value.Type == BusinessManager.BusProductToType.Elite)
                    .Where(b => b.Value.Price > 0)
                    .Select(b => b.Key)
                    .ToList();

                Log.Write(NAPI.Util.ToJson(eliteVehiclesInfo));

                if (eliteVehiclesInfo.Count < 1)
                {
                ...
                CarRoom.OpenCarromMenuGos(player, eliteVehiclesInfo, isDonate: false);
            }
            catch (Exception e)
            {
