    ...
    public class Repository
    {
        public static void UpdateStock(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You have opened a family warehouse",
                        3000);
        ...
        public static void SaveSetting(ExtPlayer player, string slogan, byte salary, string discord, int colorR, int colorG, int colorB)
        {
            try
            {
                ...
                if (slogan.Length > 65)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The slogan is too long.", 4500);
                    return;
                }
