    ...
    public class Repository
    {
        public static List<OrganizationUpdateData> Get(ExtPlayer player, OrganizationData organizationData)
        {
            ...
            if (player.IsOrganizationAccess(RankToAccess.OrgUpgrate, false))
            {
                ...
                if (upgraded == 0) 
                    updateInfo.Add(new OrganizationUpdateData("upgrade", "Office Improvement", "organisationsicon-office", false, Main.PricesSettings.FirstOrgPrice));
                else if (upgraded == 1) 
                    updateInfo.Add(new OrganizationUpdateData("upgrade", "Office Improvement", "organisationsicon-office", true, Main.PricesSettings.SecondOrgPrice));
                    
                if (!organizationData.Stock)
                    updateInfo.Add(new OrganizationUpdateData("upgrades", "Build a warehouse", "inv-safe", false, Main.PricesSettings.StockPrice));
                ...
                if (organizationData.CrimeOptions)
                {
                    foreach (var p in organizationData.Schemes)
                    {
                        if (Manager.WeaponsOrgPrice.ContainsKey(p.Key) && !p.Value)
                        {
                            ...
                            updateInfo.Add(new OrganizationUpdateData(p.Key, $"The drawing {p.Key}", Chars.Repository.ItemsInfo[wType].Icon, false, Manager.WeaponsOrgPrice[p.Key]));
                        }
                    }
                }
                ...
                {
                    updateInfo.Add(new OrganizationUpdateData("updtype", "Change the type of organization",
                        "fractionsicon-fractions", true, Main.PricesSettings.UpdateTypeOrganization));
                    
                    updateInfo.Add(new OrganizationUpdateData("newleader", "Change the leader",
