    ...
    public class Repository
    {
        ...
        public static void Disolver(ExtPlayer player)
        {
            try
            {
                ...
                if (!organizationData.IsLeader(player.GetUUID()))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not the owner of the family.", 3000);
                    return;
                }
                
                if (organizationData.Vehicles.Count >= 1)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"To dissolve the family you must sell all of the family's vehicles", 3000);
                    return;
                }
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    if (foreachPlayer != player) 
                        Notify.Send(foreachPlayer, NotifyType.Warning, NotifyPosition.BottomCenter, $"You were kicked out of your family {organizationData.Name}", 3000);
                }
                ...
                EventSys.SendCoolMsg(player,"Family", "The family is disbanded!", $"You disbanded the family, the money was handed out.", "", 10000);
