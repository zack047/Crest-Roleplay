    ...
    public class Repository
    {
        ...
        public static void UpdateRankScore(ExtPlayer player, int id, int score)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Minimum value: {NeptuneEvo.Table.Repository.MinScore}, and the maximum: {NeptuneEvo.Table.Repository.MaxScore}", 5500);
                    return;
                }
        ...
        public static void DeletePlayer(ExtPlayer player, int uuid)
        {
            try
            {
                ...
                else
                {
                    ...
                    Logs.Repository.AddLogs(player, OrganizationLogsType.UnInvite, $"Retrieved {targetMemberOrganizationData.Name} ({targetMemberOrganizationData.UUID})");
        ...
        public static void DefaultRanks(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You reset the ranks", 5000);
                var ranksName = NeptuneEvo.Table.Repository.GetRanksData(organizationData.Ranks);
                Trigger.ClientEvent(player, "client.org.main.setRanks", JsonConvert.SerializeObject(ranksName));
            }
            catch (Exception e)
        ...
        public static void SetLeader(ExtPlayer player, int uuid)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have enough RedBucks to change the leader.", 3000);
                    return;
                }
                UpdateData.RedBucks(player, -Main.PricesSettings.UpdateOrganizationLeader, msg:"Change of leader");
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You passed the leadership of the organization to a player {targetMemberOrganizationData.Name}", 5000);
