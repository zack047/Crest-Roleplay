    ...
    class Manager : Script
    {
        ...
        public static void onResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(685, "Family", new Vector3(-773.8601, 312.2175, 84.57813), 4, true));
                ...
                //
                NAPI.TextLabel.CreateTextLabel("~w~In the garage", new Vector3(-1000.14856, -774.0971, 61.734413), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                //
                NAPI.TextLabel.CreateTextLabel("~w~In the garage", new Vector3(-1000.14856, -774.0971, 60.734413), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                NAPI.Marker.CreateMarker(1, new Vector3(-1000.14856, -774.0971, 60.734413), new Vector3(), new Vector3(), 2f, new Color(255, 255, 255, 60));
                CustomColShape.CreateCylinderColShape(new Vector3(-1000.14856, -774.0971, 60.734413), 2f, 2f, NAPI.GlobalDimension, ColShapeEnums.Organizations, 5);
                //
                NAPI.TextLabel.CreateTextLabel("~w~In the garage", new Vector3(-1000.14856, -774.0971, 60.734413), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                //Office Exit
                NAPI.TextLabel.CreateTextLabel("~w~Outside", new Vector3(-1003.197, -774.2839, 61.8944), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                PedSystem.Repository.CreateQuest("ig_jackie", new Vector3(-773.945, 313.0294, 85.70606), 177.9122f, 0,"npc_org", ColShapeEnums.OrgCreate, "~y~NPC~w~ Polly\n Director of the Organization", false);
                ...
                foreach (DataRow Row in result.Rows)
                {
                    ...
                    {
                        if (stock) 
                            organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Composition {name}"), StockPositions[upgrade], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
                        else 
                            organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Composition {name}\n~r~Не активен"), StockPositions[upgrade], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
        ...
        public static void DeleteVehicle(ExtPlayer player, string number, bool type)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Such transportation is no longer in your family.", 3000);
                    return;
                }
                if (!type)
                {
                    ...
                    Trigger.ClientEvent(player, "openDialog", "ORGCAR_SELL_TOGOV", $"Do you really want to sell to the state {model} ({number}) for ${MoneySystem.Wallet.Format(price)}?");
                }
                else
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You sold {model} ({number}) for {MoneySystem.Wallet.Format(price)}$", 3000);
                    Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SellCar, $"Sold {model} ({number})");
        ...
        public static void CMD_setfamily(ExtPlayer player, int orgId, int status) {
            try
            {
                ...
                if (organizationData == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"There is no such family", 3000);
                    return;
                }
                
                if (!organizationData.Status)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This family is not active", 3000);
                    return;
                }
                ...
                if (status == 1)
                {
                    if (organizationData.CrimeOptions)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This family already has active illegal opportunities.", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have included illegal opportunities for the family #{orgId}.", 3000);
                }
                else if (status == 0)
                {
                    if (!organizationData.CrimeOptions)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This family has illegal options disabled.", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have turned off illegal family options #{orgId}.", 3000);
                }
        ...
        public static void CMD_SetOrganizationBlip(ExtPlayer player, int orgId, int sprite, byte color)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This Sprite cannot be applied", 3000);
                    return;
                }
                if (color  < 1 || color > 85)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This color cannot be applied", 3000);
                    return;
                }
                if(organizationData.Blip != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The family already has Blip", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"A tag for the family {organizationData.Name} installed.", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_DeleteOrganizationBlip(ExtPlayer player, int orgId)
        {
            try
            {
                ...
                if (organizationData.Blip == null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The family has no Blip.", 3000);
                    return;
                }
                Trigger.SetTask(async () =>
                {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Family tag {organizationData.Name} deleted.", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void SetFracRankOffline(ExtPlayer player, int uuid, int rank)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"No player is found in your family.", 3000);
                    return;
                }
                ...
                if (targetMemberOrganizationData.Rank > rank) Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SetRank, $"Downgraded the offline {targetMemberOrganizationData.Name} ({memberOrganizationData.UUID}) in the capacity of ({targetMemberOrganizationData.Rank} -> {rank})");
                else Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SetRank, $"Raised Offline {targetMemberOrganizationData.Name} ({memberOrganizationData.UUID}) in the capacity of ({targetMemberOrganizationData.Rank} -> {rank})");

                Player.Repository.SetRank(organizationData.Id, uuid, rank);
                
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed the rank of a player {targetMemberOrganizationData.Name} at {organizationData.Ranks[rank].Name}", 6000);
        ...
        public static void SetFracRank(ExtPlayer player, ExtPlayer target, int newRank)
        {
            try
            {
                ...
                if (targetMemberOrganizationData.Rank > newRank) Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SetRank, $"Reduced {target.Name} ({targetMemberOrganizationData.UUID}) в должности ({targetMemberOrganizationData.Rank} -> {newRank})");
                else Table.Logs.Repository.AddLogs(player, OrganizationLogsType.SetRank, $"Increased {target.Name} ({targetMemberOrganizationData.UUID}) in the capacity of ({targetMemberOrganizationData.Rank} -> {newRank})");
                ...
                Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"Now you {organizationData.Ranks[newRank].Name} in the family", 6000);
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed the rank of a player {target.Name} at {organizationData.Ranks[newRank].Name}", 6000);
        ...
        public static void InviteToOrganization(ExtPlayer sender, ExtPlayer target)
        {
            try
            {
                ...
                if (memberOrganizationData == null)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not a member of a family", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "The player is too far away from you", 3000);
                    return;
                }
                //if (Members.ContainsKey(target) || Fractions.Manager.isHaveFraction(target))
                if (target.IsOrganizationMemberData())
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "The player is already in the family", 3000);
                    return;
                }
                if (targetCharacterData.AdminLVL >= 1 && targetCharacterData.AdminLVL <= 7) return;
                if (targetCharacterData.LVL < 1)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "At least 1 level is required to invite a player into the family", 3000);
                    return;
                }
                if (targetCharacterData.Warns > 0)
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, "It is impossible to accept this player", 3000);
                    return;
                }
                targetSessionData.InviteData.Organization = organizationData.Id;
                targetSessionData.InviteData.Sender = sender;
                Trigger.ClientEvent(target, "openDialog", "INVITED_ORG", $"{sender.Name} invited you into the family {organizationData.Name}");
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"You invited the family {target.Name}", 3000);
        ...
        public static void UnInviteFromOrganization(ExtPlayer sender, ExtPlayer target)
        {
            try
            {
                ...
                {
                    Notify.Send(sender, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't kick this player out.", 3000);
                    return;
                }
                
                NeptuneEvo.Organizations.Player.Repository.RemoveOrganizationMemberData(memberOrganizationData.Id, target.GetUUID());

                Notify.Send(target, NotifyType.Warning, NotifyPosition.BottomCenter, $"You were kicked out of your family {organizationData.Name}", 3000);
                Notify.Send(sender, NotifyType.Success, NotifyPosition.BottomCenter, $"You kicked out of the family {target.Name}", 3000);
                Table.Logs.Repository.AddLogs(sender, OrganizationLogsType.UnInvite, $"Retrieved {target.Name} ({target.GetUUID()})");
            }
            catch (Exception e)
            {
        ...
        public static bool StockBuy(ExtPlayer player)
        {
            try
            {
                ...
                if (organizationData == null) 
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are not a member of a family", 3000);
                    return false;
                }
                if (organizationData.Stock)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The warehouse is already set up for your family.", 3000);
                    return false;
                }
                ...
                organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Composition {organizationData.Name}"), StockPositions[organizationData.OfficeUpgrade], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a storage unit for the family!", 3000);
        ...
        public static bool UpdateCrimeOptions(ExtPlayer player) 
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You don't have enough RedBucks to improve your family.", 3000);
                    return false;
                }
                UpdateData.RedBucks(player, -Main.PricesSettings.UpdateTypeOrganization, msg:"Improving the family");
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have changed the type of organization!", 3000);
                return true;
            }
            catch (Exception e)
            {
        ...
        public static bool SchemeBuy(ExtPlayer player, string scheme, int price = -1)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This pattern is already bought into your family.", 3000);
                    return false;
                }
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought the blueprint for the family!", 3000);
        ...
        public static bool UpgradeOrganization(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You should not be in a vehicle during the purchase of the improvement.", 3000);
                    return false;
                }
                if (upgraded == 0)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You don't have enough money to improve your family.", 3000);
                        return false;
                    }
                ...
                if (organizationData.Stock) 
                    ...
                    organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Composition {organizationData.Name}\n~r~Not active"), StockPositions[organizationData.OfficeUpgrade], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have improved the family!", 3000);
                GameLog.Money($"player({player.GetUUID()})", $"server", Main.PricesSettings.StockPrice, $"OrgStockBuy({organizationData.Id})");
                return true;
            }
            catch (Exception e)
        ...
        public static void CreateOrganization(ExtPlayer player, bool isCrime, string orgName)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Not available for administration.", 3000);
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You must have no warnings (Warns) in order to start a family.", 3000);
                    Trigger.ClientEvent(player, "client.org.create.close");
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Unfortunately, the family with the name {orgName} already exists.", 3000);
                    return;
                }
                if (!MoneySystem.Wallet.Change(player, -Main.PricesSettings.CreateOrgPrice))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have enough money to start a family.", 3000);
                    Trigger.ClientEvent(player, "client.org.create.close");
                    return;
                }
                    ...
                    try
                    {
                        ...
                        Trigger.SetMainTask(() =>
                        {
                            try
                            {
                                ...
                                CustomColShape.CreateCylinderColShape(new Vector3(-1396.274, -480.7186, 57.100), 2f, 2f, organizationData.GetDimension(), ColShapeEnums.Organizations, 4);
                                organizationData.StockLabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"~y~Composition {orgName}\n~r~Not active"), StockPositions[0], 5F, 0.5F, 0, new Color(255, 255, 255), true, organizationData.GetDimension());
                                ...
                                SendPlayer(player);
                                NAPI.Chat.SendChatMessageToAll("!{#50C878}" + $"A new family has been registered in the state - {orgName}!");
                                
                                EventSys.SendCoolMsg(player,"Family", "Creating a Family", $"Congratulations on registering your own family - {orgName}!", "", 10000);
        ...
        public static void CraftBodyArmor(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "Not available to your family.", 5000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have crafted an armor vest with 100% strength.", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void OpenStock(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "The family warehouse is closed.", 3000);
                    return;
                }
                OpenOrgStock(player);
            }
                ...
                switch (action)
                {
                    case "put_stock":
                        ...
                        if (item == "mats")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMats, $"Put materials in the warehouse (x{amount})");
                        }
                        else if (item == "drugs")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeDrugs, $"He put the drugs in storage (x{amount})");
                        }
                        else if (item == "money")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMoney, $"Put the money in the warehouse (${MoneySystem.Wallet.Format(amount)})");
                        }
                        else if (item == "medkits")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMedkits, $"I put the first aid kits in storage (x{amount})");
                        }
                        //GameLog.Stock(cdata.FractionID, cdata.UUID, player.Name, item, amount, "in");
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Remaining in stock {stockContains + amount}, you have {playerHave - amount}", 3000);
                        break;
                    case "take_stock":
                        ...
                        if (item == "mats")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMats, $"Took materials from the warehouse (x{amount})");
                        }
                        else if (item == "drugs")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeDrugs, $"He took drugs from the warehouse (x{amount})");
                        }
                        else if (item == "money")
                        {
                            var lastMulti = organizationData.MoneyMultiplier();
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMoney, $"Took money from the warehouse (${MoneySystem.Wallet.Format(amount)})");
                        }
                        else if (item == "medkits")
                        {
                            ...
                            Table.Logs.Repository.AddLogs(player, OrganizationLogsType.TakeMedkits, $"Got the first aid kits from the warehouse (x{amount})");
                        }
        ...
        public static void Open(ExtPlayer player, int index)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You're dragging someone", 3000);
                return;
            }
        ...
        public static void OnOrganizations(ExtPlayer player, int intid)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are dragging someone", 3000);
                    return;
                }
                switch (intid)
                {
                    case 1:
                        ...
                        Trigger.ClientEvent(player, "openDialog", "CONFIRM_BUY_ORGBODYARMOUR", "Do you really want to craft a 100% body armor worth 400 materials?");
                        return;
                    case 2:
                        OpenStock(player);
                        return;
            ...
            try
            {
                ...
                switch (listItem)
                {
                    ...
                    case "createorg":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You are already in the family.", 5000);
                            return;
                        }
                        Trigger.ClientEvent(player, "openDialog", "CreateOrg", $"Are you sure you want to create your family? The name of the family is your last name, when you create it for {Main.PricesSettings.CreateOrgPrice}$ you get a 5k garage and an entry level office, as well as a family chat (/fc /fcb).");
                        break;
                }
            }
            catch (Exception e)
        ...
        public static void organizationChat(ExtPlayer player, string message)
        {
            try
            {
                ...
                string status = organizationData.Ranks.ContainsKey(memberOrganizationData.Rank) ? $"{organizationData.Ranks[memberOrganizationData.Rank].Name} " : "No ";
