...
namespace NeptuneEvo.PedSystem.Pet
{
    class Repository
    {
        ...
        public static void LoadInGame()
        {
            ...
            PedSystem.Repository.CreateQuest("a_m_y_soucent_03", new Vector3(268.109, -641.3529, 42.01984), 60.41674f, title: "~y~NPC~w~ Vitaly\nPet seller", colShapeEnums: ColShapeEnums.PetShop);
        ...
        public static void AddBall(ExtPlayer player)
        {
            ...
            if (characterData == null)
                return;
            Commands.RPChat("do", player, $"A pet brought a ball.");
            ...
            try
            {
                ...
                Timers.StartOnce(5000, () =>
                {
                    try
                    {
                        ...
                        Damage(ped, eatData);
                        Commands.RPChat("sme", player, $"fed" + (characterData.Gender ? "" : "а") + LangFunc.GetText(LangType.Ru, DataName.Pets));
                    }
                    catch (Exception e)
                    {
