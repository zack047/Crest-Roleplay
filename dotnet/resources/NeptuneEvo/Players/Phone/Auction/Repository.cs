...
namespace NeptuneEvo.Players.Phone.Auction
{
    public class Repository
    {
        ...
        public static void OnCreate(ExtPlayer player, AuctionType type, int elementId, string text, string image, int price)
        {
            ...
            else if (type == AuctionType.Other)
            {
                ...
                if (Chars.Repository.ItemsData.ContainsKey(locationName) && Chars.Repository.ItemsData[locationName].ContainsKey("inventory"))
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Subject not found", 3000);
                        return;
                    }
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have successfully created a lot", 3000);
        }
        ...
        private static void Insert(ExtPlayer player, AuctionType type, int elementId, string title, string text, string image, int price)
        {
            Trigger.SetTask(async () =>
            {
                try
                {
                    ...
                    GameLog.Money($"player({characterData.UUID})", $"server", price, $"auction [#{autoId}]: Creating a lot ({type.ToString()})");
        ...
        private static void End(AuctionData auctionData)
        {
            ...
            Messages.Repository.AddSystemMessageToUuid(auctionData.CreateUUID, (int) DefaultNumber.Auction, $"Auction on {auctionData.Title} finished.", DateTime.Now);
        ...
        public static void OnBet(ExtPlayer player, int id, int price)
        {
            ...
            {
                Init(player);
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Lot not found...", 3000);
                return;
            }
            
            if (auctionData.CreateUUID == characterData.UUID)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot bid on your lot.", 3000);
                return;
            }

            if (auctionData.LastBetUUID == characterData.UUID)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The last bet is already yours.", 3000);
                return;
            }
            if (auctionData.LastPrice >= price)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The rate cannot be less than the current rate.", 3000);
                return;
            }
            ...
            if (auctionData.Type == AuctionType.House)
            {
                ...
                if (house != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You already have a home.", 3000);
                    return;
                }
            ...
            else if (auctionData.Type == AuctionType.Biz)
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You already have a business.", 3000);
                    return;
                }
            }
            ...
            GameLog.Money($"player({characterData.UUID})", $"auction(Bet)", priceAndPercent, $"auction [#{auctionData.Id}]: Rate ({auctionData.Type.ToString()})");
            ReturnMoney(auctionData.LastBetUUID, auctionData.LastPrice, auctionData.Id, auctionData.Type, "Your bet has been outbid!");
            ...
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You made a bet..", 3000);
            //Update(auctionData);
            if (!isAddMyList)
            {
