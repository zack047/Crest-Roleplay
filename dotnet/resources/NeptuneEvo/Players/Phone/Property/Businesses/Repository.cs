...
namespace NeptuneEvo.Players.Phone.Property.Businesses
{
    public class Repository
    {
        ...
        public static void MaxProducts(ExtPlayer player)
        {
            ...
            var message = "Do you really want to buy:<br/>";
            ...
            Trigger.ClientEvent(player, "openDialog", "BIZ_CONFIRM_BUY_ORDERS", $"You wish to order goods in quantity <font style='color: red'>{whCount}</font> amounting to <font style='color: green'>{whCost}$</font>.<br/><br/>{message}");
        }
        ...
        public static void CancelOrder(ExtPlayer player, int uid)
        {
            ...
            if (order == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You did not order this product", 3000);
                return;
            }
            ...
            if (order.Taked)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot cancel an order while it is being delivered", 3000);
                return;
            }
            ...
            biz.Zatratq -= price;
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You canceled your order at {order.Name}", 3000);
        }
        ...
        public static void ExtraCharge(ExtPlayer player, string name, int value)
        {
            ...
            if (name == "Лотерейный билет" || name == "Consumables")
            {
            ...
            var ch = (biz.Type == 7 || biz.Type == 11 || biz.Type == 12 || product.Name == "Tattoos" || product.Name == "Wigs") ? "%" : "$";
        ...
        public static void OnSell(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "openDialog", "BIZ_SELL_TOGOV", $"Do you really want to sell real estate for ${MoneySystem.Wallet.Format(price)}?");   
        }
