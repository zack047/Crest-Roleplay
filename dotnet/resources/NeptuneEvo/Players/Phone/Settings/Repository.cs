    ...
    public class Repository
    {
        public static int OnAddSim(ExtPlayer player, int newSim)
        {
            ...
            if (sim != -1)
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "The SIM card has been replaced.", 3000);
        ...
        public static void OnRemoveSim(ExtPlayer player)
        {
            ...
            if (sim == -1)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "SIM card is not inserted", 3000);
                return;
            }
        ...
        public static void OnUpdateAir(ExtPlayer player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Air mode can be changed every 5 minutes", 3000);
                return;
            }
        ...
        public static void OnUpdateForbesVisible(ExtPlayer player)
        {
            ...
            Trigger.ClientEvent(player, "phone.notify", (int) DefaultNumber.RedAge, $"Forbes privacy status will change with the next update of the top.", 5);
        }
