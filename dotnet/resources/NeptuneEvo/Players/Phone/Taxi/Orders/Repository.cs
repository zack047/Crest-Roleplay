    ...
    {        
        ...
        public static void OnUpdatePos(ExtPlayer player, ExtPlayer target)
        {
            ...
            if (player.Position.DistanceTo2D(target.Position) < 10f)
            {
                ...
                Trigger.ClientEvent(player, "phone.notify", (int) DefaultNumber.Taxi, $"Your cab's here!", 8); 
        ...
        public static void TaxiPay(ExtPlayer player, int mileage)
        {
            ...
            {
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "The player does not have enough money to pay.", 6000);
                return;
            }
