    ...
    class Repository
    {
            ...
            try
            {
                ...
                if (getSession == SessionEnum.Error || getSession == SessionEnum.NoAccount)
                {
                    ...
                    if (account != null)
                    {
                        ...
                        if (ban != null)
                        {
                            player.setBan($"You are blocked {ban.Time.Day} {ban.Time.ToString("MMMM")} {ban.Time.Year}s. {ban.Time.Hour}:{ban.Time.Minute}:{ban.Time.Second} To {ban.Until.Day} {ban.Until.ToString("MMMM")} {ban.Until.Year}s. {ban.Until.Hour}:{ban.Until.Minute}:{ban.Until.Second} (UTC+3). Reason: {ban.Reason} ({ban.Byadmin})");
                            return;
                        }
        ...
        public static async Task PlayerToAuntidication(ExtPlayer player)
        {
            ...
            /*if (!IsWhiteLogin(auntificationData.Login))
            {
                ...
                Notify.Send(player, NotifyType.Error, NotifyPosition.Center, $"At the moment the server is undergoing technical work, follow the news in the off. discord.", 1000 * 60);
                return;
            }*/
