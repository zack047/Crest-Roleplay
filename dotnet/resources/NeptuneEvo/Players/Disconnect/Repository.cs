            ...
            try
            {
                ...
                if (characterData != null)
                {
                    ...
                    if (type != DisconnectionType.Kicked && characterData.AdminLVL == 0)
                    {
                        ...
                        if (cufdata.Cuffed && (cufdata.CuffedByMafia || cufdata.CuffedByCop) && characterData.DemorganTime <= 0)
                        {
                            if (cufdata.CuffedByCop)
                            {
                                ...
                                GameLog.Admin($"server", $"demorgan(4h, leaving the game in handcuffs)", $"{player.Name}");
                            }
                            else if (cufdata.CuffedByMafia)
                            {
                                ...
                                GameLog.Admin($"server", $"demorgan(4h,exit from the game in screeds)", $"{player.Name}");
                            }

                            characterData.DemorganInfo.Admin = "Server";
                            characterData.DemorganInfo.Reason = "leaving the game in handcuffs or ties.";
                    ...
                    if (characterData.AdminLVL >= 1 && characterData.AdminLVL <= 5)
                        Trigger.SendToAdmins(1, $"!{{#FFB833}}[A][{characterData.UUID}]{player.Name.Replace('_', ' ')} disconnected (Level {characterData.AdminLVL})");
                    
                    Commands.RPChat("sb", player, LangFunc.GetText(LangType.Ru, DataName.PlayerDisconnect), optName: player.Name.Replace('_', ' ') + $" ({characterData.UUID})");
