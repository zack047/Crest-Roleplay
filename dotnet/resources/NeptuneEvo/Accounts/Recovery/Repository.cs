    ...
    class Repository
    {
        ...
        public static void SendEmail(ExtPlayer player, string login)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"An email with the password recovery code has been successfully sent to {Generate.ObfuscateEmail(auntificationData.Email)}.", 5000);
        ...
        public static async void RecoveryPassword(ExtPlayer player, string code)
        {
            try
            {
                ...
                {
                    Log.Debug($"{sessionData.RealSocialClub} successfully recovered the password!", nLog.Type.Info);
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You have successfully regained access to your account. New password sent to your email!", 5000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "In addition, you can change it right now from the game by entering /password and a new desired password with a space. Example [/password 123] without brackets.", 10000);
                }
                else
                {
