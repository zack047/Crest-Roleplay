                    ...
                    foreach (var hash in confirms)
                    {
                        ...
                        if (emailVerification != null && emailVerification.Player != null)
                        {
                            if (!emailVerification.IsRegistered) 
                                Notify.Send(emailVerification.Player, NotifyType.Success, NotifyPosition.BottomCenter, "The time for mail confirmation has expired. Try again!", 5000);
                            else 
                                Accounts.Registration.Repository.MessageError(emailVerification.Player, "The time for mail confirmation has expired. Try again!");
                        }
                    }
                });
            }
