                ...
                if (lobbyId >= 0 && LobbyList.ContainsKey(lobbyId) && LobbyList[lobbyId].TR_Players.ContainsKey(player))
                {
                    if (LobbyList[lobbyId].TR_State == 2)
                    {
                        ...
                        if(!dc)
                        {
                            ...
                            NAPI.Player.SpawnPlayer(player, new Vector3(-257.8227, -2020.5167, 30.145576));
                            //player.Health = 100;
                            if (killlist)
                            {
                                lock (LobbyList[lobbyId].TR_Players.Keys)
                                {
                                    foreach (ExtPlayer foreachPlayer in LobbyList[lobbyId].TR_Players.Keys)
                                    {
                                        ...
                                        NAPI.Notification.SendNotificationToPlayer(foreachPlayer, $"~r~{player.Name.Replace('_', ' ')} ~s~retired from the game", true);
                                    }
                                }
                            }
                        }
                            ...
                            if (target.Health <= 7 || PlayerInfo.Value.Health <= 5)
                            {
                                ...
                                lock (LobbyList[lobby.Key].TR_Players.Keys)
                                {
                                        ...
                                        if (LobbyList[lobby.Key].TR_Players.Keys.Count >= 1)
                                        {
                                            ...
                                            foreach (ExtPlayer foreachPlayer in Players)
                                            {
                                                ...
                                                Trigger.SendChatMessage(foreachPlayer,
                                                    "~o~[Tank Royale] The event was completed without a winner.");
                                            }
                                        }
                                    ...
                                    else if (count >= 2)
                                    {
                                        ...
                                        {
                                            if (!foreachPlayer.IsCharacterData()) continue;
                                            Trigger.SendChatMessage(foreachPlayer, $"~o~[Tank Royale] The game is left with {LobbyList[lobby.Key].TR_Players.Count} tanks!");
                                        }
                                    }
                                }
                            }
            ...
            try
            {
                ...
                if (winnerCharacterData == null)
                {
                    if (LobbyList[index].TR_Players.Keys.Count >= 1)
                    {
                        ...
                        {
                            if (!foreachPlayer.IsCharacterData()) continue;
                            Trigger.SendChatMessage(foreachPlayer, "~o~[Tank Royale] The event was completed without a winner.");
                        }
                    }
                    EndGame(index);
                    return;
                ...
                if (LobbyList[index].TR_Players.Keys.Count >= 1)
                {
                    ...
                    {
                        if (!foreachPlayer.IsCharacterData()) continue;
                        Trigger.SendChatMessage(foreachPlayer, $"~o~[Tank Royale] Winner {winner.Name.Replace('_', ' ')} survived with {LobbyList[index].TR_Players[winner].Health}HP and got {Wallet.Format(LobbyList[index].TR_Bank)}$.");
                    }
                }
                EndGame(index);
            }
                    ...
                    case 1:
                        if(LobbyList[index].TR_TimerTicks == 19) // 9:30
                        {
                            ...
                            foreach (ExtPlayer foreachPlayer in players)
                            {
                                ...
                                if (!SafeZones.IsSafeZone(foreachSessionData.InsideSafeZone, SafeZones.ZoneName.Arena)) Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter, $"If you will be outside the Green Zone of the event, you will not get into the event.", 3000);
                                else if (foreachPlayer.IsInVehicle) Notify.Send(foreachPlayer, NotifyType.Info, NotifyPosition.BottomCenter, $"If you sit in the car, you won't make it to the event.", 3000);
                            }
                        ...
                        else if(LobbyList[index].TR_TimerTicks == 20) // 10
                        {
                            if (LobbyList[index].TR_Players.Count >= 2)
                            {
                                ...
                                foreach (ExtPlayer foreachPlayer in players)
                                {
                                    ...
                                    if (!SafeZones.IsSafeZone(foreachSessionData.InsideSafeZone, SafeZones.ZoneName.Arena))
                                    {
