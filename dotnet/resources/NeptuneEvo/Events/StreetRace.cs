    ...
    class StreetRace : Script
    {
        ...
        private static void callback_selectChar(ExtPlayer player, object listItem)
        {
            try
            {
                ...
                ExtPlayer target = Main.GetPlayerByUUID(Convert.ToInt32(listItem));
            ...
            try
            {
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath)
                {
                    ...
                    if (targetSessionData != null && !targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath)
                    {
                        ...
                        EventSys.SendCoolMsg(streetRaceData.joined,"Race", "Race Challenge", $"{LangFunc.GetText(LangType.Ru, DataName.PlayerOFferGonka, player.Value, streetRaceData.rate)}", "", 10000);
                        EventSys.SendCoolMsg(player,"Race", "Race Challenge", $"{LangFunc.GetText(LangType.Ru, DataName.YouOfferGonka, streetRaceData.joined.Value)}", "", 10000);
        ...
        public static void StreetRaceFinished(ExtPlayer player)
        {
            try
            {
                ...
                EventSys.SendCoolMsg(player,"Race", "Victory!", $"{LangFunc.GetText(LangType.Ru, DataName.YouWinGonka, cost)}", "", 10000);
                ...
                EventSys.SendCoolMsg(LosePlayer,"Race", "Defeat :(", $"{LangFunc.GetText(LangType.Ru, DataName.YouLoseGonka, cost)}", "", 10000);
