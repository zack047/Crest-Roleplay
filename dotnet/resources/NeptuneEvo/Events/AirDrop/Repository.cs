...
namespace NeptuneEvo.Events.AirDrop
{
    public class Repository : Script
    {
        ...
        {
            PedSystem.Repository.CreateQuest("g_m_m_cartelguards_01", new Vector3(-54.35258, -1213.5707, 28.684637), 133.74596f, title: "~y~NPC~w~ Juan de Cartel\nAirDrop Informant", colShapeEnums: ColShapeEnums.PedAirDrop);
        }
            ...
            try
            {
                ...
                if (hour == 8 || hour == 15 || hour == 21 || needStart)
                {
                    ...
                    foreach (var foreachPlayer in Character.Repository.GetPlayers())
                    {
                        ...
                        EventSys.SendCoolMsg(foreachPlayer,"AirDrop", $"It's about to start!", LangFunc.GetText(LangType.Ru, DataName.AirDropZakazan), "", 10000);
                        ...
                        Trigger.ClientEvent(foreachPlayer, "phone.notify", (int)DefaultNumber.Informer, $"Gringo!!! Boxes are expected to drop within 20 minutes, there will be: {dropCount}. Write /airdrop if you're having trouble getting it.", 20);
                    }
            ...
            try
            {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    try
                    {
                        ...
                        Trigger.ClientEvent(foreachPlayer, "phone.notify", (int) DefaultNumber.Informer, $"Hola! I heard through my channels that AirDrop is over. Hope all is going well for you, see you again!", 15);
        ...
        public static void Perform(ExtPlayer player)
        {
            try
            {
                ...
                {
                    string nextAirDropTime = "The next AirDrop will be available at " + (AirDropEndEventTime.Hour < 10 ? $"0{AirDropEndEventTime.Hour}" : $"{AirDropEndEventTime.Hour}") + ":" + (AirDropEndEventTime.Minute < 10 ? $"0{AirDropEndEventTime.Minute}" : $"{AirDropEndEventTime.Minute}");
