    ...
    class MafiaGame : Script
    {
        ...
        public static void CreateMafiaLobby(ExtPlayer player, string lobby_name, int lobby_price, string lobby_password, int gameMap)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "You need at least 6 participants in the lobby for the game to start. If you leave the area, the game will be cancelled.", 5000);
                ...
                else if (LobbyList[index].GameStatus == 2)
                {
                    ...
                    foreach (ExtPlayer MafiaPlayer in LobbyList[index].MafiaPlayers)
                    {
                        try
                        {
                            ...
                            {
                                SendTextInfoForPlayer(MafiaPlayer, $"Your role in this game: {role_names[MafiaPlayer.GetSharedData<int>("mafiaGameRole")]}", 10000);
                                ...
                                if (MafiaPlayer.IsCharacterData())
                                {
                                    ...
                                    if (MafiaPlayer.GetSharedData<int>("mafiaGameRole") == 3)
                                    {
                                        if (LobbyList[index].wasHealed == -1)
                                            SendTextInfoForPlayer(MafiaPlayer, "Unfortunately, time is up. You have not cured anyone.");
