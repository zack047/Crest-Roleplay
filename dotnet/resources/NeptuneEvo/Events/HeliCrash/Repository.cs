...
namespace NeptuneEvo.Events.HeliCrash
{
    public class Repository
    {
        ...
        {
            {HeliType.Medical, "rescue"},
            {HeliType.Army, "Military"}
        };
        ...
        public static void Create()
        {
            try
            {
                ...
                foreach (var foreachPlayer in Character.Repository.GetPlayers())
                {
                    ...
                    Trigger.SendChatMessage(foreachPlayer, "~o~[HeliCrash] Ten minutes later over " +
                                                           $"{(foreachSessionData.IsRadioInterceptor ? _heliCords.Name : "by the state")} fly by {HeliDisplayName[_heliType]} helicopter, which transmits a distress signal.");
                    EventSys.SendCoolMsg(foreachPlayer,"HeliCrash", $"In 10 minutes!", $"Above. {(foreachSessionData.IsRadioInterceptor ? _heliCords.Name : "by the state")} fly by {HeliDisplayName[_heliType]} helicopter, which transmits a distress signal.", "", 10000);
                }
        ...
        private static (int, int) GetHealth(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Someone is already hacking", 3000);
                    return (0, 0);
                }
