...
namespace NeptuneEvo.Events
{
    ...
    class Festive : Script
    {
        ...
        public static void Init ()
        {
            if (isEvent)
            {
                PedSystem.Repository.CreateQuest("dedmoroz", new Vector3(-1640.6166, -1096.9696, 13.024864), -42f, title: "~y~NPC~w~ Santa Claus\nGift exchange", colShapeEnums: ColShapeEnums.Festive);
                Main.CreateBlip(new Main.BlipData(607, "Santa Claus", new Vector3(-1640.6166, -1096.9696, 13.024864), 0, true));
        ...
        public static void OnFestive(ExtPlayer player)
        {
            try
            {
                ...
                Trigger.ClientEvent(player, "openInput", $"Обмен {EventCoinsName} | 3 = 1 RB", "Enter numbers divisible by 3 - 3, 6, 9, etc.", 3, "sell_festive");
            }
            catch (Exception e)
            {
        ...
        public static void EventsCollect(ExtPlayer player, int index)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You've already assembled this box, look for others!", 8000);
                    return;
                }
        ...
        public static void CMD_givehc(ExtPlayer player, int id, int amount)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You gave the man {target.Name} {amount} Gifts", 5000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_givehcrad(ExtPlayer player, int radius, int amount)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have given out to players in the radius {radius} {amount} Gifts", 6000);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (index)
                {
                    case 0:
                        if (99999 > count)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You don't have that many gifts.", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You successfully exchanged your gifts for Christmas pants", 6000);
                        break;
                    case 1:
                        if (650 > count)
                        {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You successfully exchanged your gifts for a Christmas top", 6000);
                        break;
                    case 2:
                        if (1000 > count)
                        {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have successfully exchanged your gifts for lollipops", 6000);
                        break;
                    case 3:
                        if (650 > count)
                        {
                    ...
                    case 6:
                        if (1500 > count)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You don't have that many coins", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have successfully exchanged your gifts for a snowmobile!", 6000);
                        break;
                    default:
                        return;
                }
