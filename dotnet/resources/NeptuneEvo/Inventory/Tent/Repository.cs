            ...
            try
            {
                ...
                switch (listItem)
                {
                    ...
                    case "rentTime":
                        ...
                        // Check if the rent time is not already at the maximum (24 hours)
                        if (tentData.RentTime < DateTime.Now.AddHours(24))
                        {
                            EventSys.SendCoolMsg(player, "Marketplace", "Rent a tent", $"{LangFunc.GetText(LangType.Ru, DataName.RentTentExpand)}", "", 7000);
                        ...
                        {
                            // Notify the player that the rent time cannot be extended beyond 24 hours
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You cannot extend the rent time beyond 24 hours.", 3000);
                        }
                        break;
                    case "name":
