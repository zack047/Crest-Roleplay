        ...
        public static async Task Load(ExtPlayer player, int uuid, int spawnid)
        {
            try
            {
                ...
                if (GameLog.OnlineQueue.ContainsKey(uuid))
                {
                    ...
                    GameLog.Disconnected(uuid, player.Value, "Deleting the old connection (Bug)", accountData.Login);       
                    //return;
                }
                ...
                if (ban)
                {
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "You won't get through!", 4000);
                    return;
                }
                ...
                if (character != null)
                {
                    ...
                    NAPI.Task.Run(() =>
                    {
                        ...
                        player.SetSharedData("DENI_UUID2", characterData.UUID);
                    });
                    ...
                    if (characterData.AdminLVL >= 1 && characterData.AdminLVL <= 5)
                        Trigger.SendToAdmins(1, $"!{{#FFB833}}[A] {fullname} logged in ({characterData.AdminLVL} lvl)");
