    ...
    class FunctionsAccess : Script
    {
        ...
        public void CMD_Refreshclothes(ExtPlayer player)
        {
            try
            {
                ...
                UpdateSystemState();
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have updated .", 3000);
            }
            catch (Exception e)
            {
        ...
        public void CMD_Enablefunc(ExtPlayer player, string name, bool toogled)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have changed the access to the function {name} at {toogled}", 10000);
            }
            catch (Exception e)
            {
