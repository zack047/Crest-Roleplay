...
namespace NeptuneEvo.Functions
{
    ...
    {
        public static bool RecentlyTeleported(this ExtPlayer player)
        {
            return player.GetData<bool>("RecentlyTeleported");
        }
        public static void SetTeleported(this ExtPlayer player, bool value)
        {
            player.SetData<bool>("RecentlyTeleported", value);
        }
    }
    class CustomColShape : Script
    {
        ...
        public void OnPlayerEnterColshape(ExtColShape shape, ExtPlayer player)
        {
            try
            {
                if (player.RecentlyTeleported()) return;
                ...
                if (sData.ColShapeId == ColShapeEnums.WarehouseEnter)
                {
                    Stock.WarehouseEnter(player);
                    player.SetTeleported(true);
                    NAPI.Task.Run(() => player.SetTeleported(false), delayTime: 2000);
                }
                else if (sData.ColShapeId == ColShapeEnums.WarehouseExit)
                {
                    Stock.WarehouseExit(player);
                    player.SetTeleported(true);
                    NAPI.Task.Run(() => player.SetTeleported(false), delayTime: 2000);
                }
                else
