...
namespace NeptuneEvo.Houses
{
    ...
    public class House
    {
        ...
        public House(int id, string owner, int type, Vector3 position, int price, bool locked, int garageID, int bank, Dictionary<string, ResidentData> roommates, int dimensionID, bool healkit, bool alarm)
        {
            ...
            label = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"Real Estate {id}"), position + new Vector3(0, 0, 1.5), 5f, 0.4f, 0, new Color(255, 255, 255), false, 0);
            UpdateLabel();
            UpdateColShapeHealkit();
            UpdateGarage();
            
            if (Main.ServerSettings.IsHouseBlips)
                blip = (ExtBlip) NAPI.Blip.CreateBlip(374, position, 0.45f, 82, "Home", shortRange: true);
        }
            ...
            try
            {
                if (Type != 7)
                {
                    if (Healkit)
                    {
                        ...
                        Healkitlabel = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~First Aid Kit"), HouseManager.HouseHealkitPos[Type - 1], 5f, 0.3f, 0, new Color(255, 255, 255), false, (uint)Dimension);
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(Owner))
                    {
                        ...
                        if (GarageID != 0 && GarageManager.Garages.ContainsKey(GarageID)) 
                            text += $"~w~Garage: ~o~{GarageManager.GarageTypes[GarageManager.Garages[GarageID].Type].MaxCars} Cars\n";
                        if (Type != 7) text += (Locked) ? "~r~Closed~c~\n" : "~g~Open~c~\n";
                    }
                    else if (IsAuction)
                    {
                        text += $"~w~Up for auction\n";
                        if (GarageID != 0 && GarageManager.Garages.ContainsKey(GarageID)) 
                            text += $"~w~Garage: ~o~{GarageManager.GarageTypes[GarageManager.Garages[GarageID].Type].MaxCars} Cars\n";
                    }
                    else
                    {
                        text += $"~w~Price.: ~g~{MoneySystem.Wallet.Format(Price)}$\n";
                        if (GarageID != 0 && GarageManager.Garages.ContainsKey(GarageID)) 
                            text += $"~w~Garage: ~o~{GarageManager.GarageTypes[GarageManager.Garages[GarageID].Type].MaxCars} Cars\n";
                    }
                    text += $"~c~ID{ID}";
        ...
        public void CreateFurniture(HouseFurniture f)
        {
            try
            {
                ...
                switch (f.Name)
                {
                    ...
                    case "Clothes closet":
                    case "Filing cabinet":
                    case "Burglary-proof safe":
                        if (f.Name == "Gun safe") name = "WeaponSafe";
                        else if (f.Name == "Clothes closet") name = "ClothesSafe";
                        else if (f.Name == "Filing cabinet") name = "SubjectSafe";
                        else if (f.Name == "Burglary-proof safe") name = "BurglarProofSafe";
            ...
            Trigger.SetMainTask(() =>
            {
                try
                {
                    ...
                    foreach (var name in names)
                    {
                        ...
                        {
                            if (ownerName != name) 
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "You were evicted from your home", 3000);
    ...
    class HouseManager : Script
    {
            ...
            try
            {
                ...
                switch (action)
                {
                    case "buy":
                        ...
                        if (!string.IsNullOrEmpty(house.Owner))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The property already has an owner", 3000);
                            return;
                        }
                        if (house.IsAuction)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't go inside the house because it's up for auction.", 7000);
                            return;
                        }
                        if (Players.Phone.Auction.Repository.IsBet(characterData.UUID, AuctionType.House))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot purchase the property because your home is at auction.", 6000);
                            return;
                        }
                        if (house.Price == 0 && characterData.AdminLVL <= 5)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The house is not available for purchase.", 3000);
                            return;
                        }
                        if (house.Price > characterData.Money)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have enough money to buy", 3000);
                            return;
                        }
                        if (Houses.Count(h => h.Owner == player.Name) >= 1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot buy more than one property", 3000);
                            return;
                        }
                    ...
                    case "int":
                        ...
                        if (house.Type != 7)
                        {
                            if (!string.IsNullOrEmpty(house.Owner))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This house already has an owner", 3000);
                                return;
                            }
                            house.SendPlayer(player);
                        }
                        else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The parking space has no interior", 3000);
                        return;
                }
            }
            catch (Exception e)
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case "removeall":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be in your home for this action", 3000);
                            return;
                        }
                        if (house.Removedall > DateTime.Now)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You recently kicked players out of the house, the next time can be used within 5 minutes.", 3000);
                            return;
                        }
                        house.Removedall = DateTime.Now.AddMinutes(5);
                        house.RemoveAllPlayers(player);
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You kicked everyone out of the house", 3000);
                        return;
                    case "sell":
                        int price = 0;
                        ...
                        Trigger.ClientEvent(player, "openDialog", "HOUSE_SELL_TOGOV", $"Do you really want to sell real estate for ${MoneySystem.Wallet.Format(price)}?");                        
                        return;
                }
            }
            catch (Exception e)
        ...
        public static void acceptHouseSellToGov(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have to come out of the garage", 3000);
                    return;
                }
        ...
        private void GarageUpdateLvl(ExtPlayer player)
        {
            try
            {
                ...
                if (garage != null && garage.Type != 6)
                {
                    if (garage.Type == 9)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Your home has the largest number of garage spaces.", 3000);
                        return;
                    }
                    ...
                    else
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You do not have enough RedBucks.", 3000);
                            return;
                        }
                        UpdateData.RedBucks(player, -garageType.Price, msg: "Garage Improvement");
                    }
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have successfully improved your garage to {garageType.MaxCars} seats.", 5000);
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Any garage improvements will be reset when the house is sold, remember that!", 5000);
                }
        ...
        private void OnHouseFurnitureBuy(ExtPlayer player, string name)
        {
            try
            {
                ...
                if(name == "Home First Aid Kit" || name == "Alarm")
                {
                    if (characterData.InsideHouseID == -1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be at home for this action", 3000);
                        return;
                    }
                    var house = GetHouse(player, true);
                    if (house == null)
                    ...
                    if (name == "Home First Aid Kit")
                    {
                        if (house.Type == 0 || house.Type == 7) return;
                        if (house.Healkit)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have already purchased this improvement.", 3000);
                            return;
                        }
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", 2500, $"buyFurn({house.ID} | Home First Aid Kit)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Congratulations on your successful purchase of a home medicine cabinet!", 3000);
                    }
                    else if (name == "Alarm")
                    {
                        if (house.Type == 0 || house.Type == 7) return;
                        ...
                        GameLog.Money($"player({characterData.UUID})", "server", 3000, $"buyFurn({house.ID} | Alarm)");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Congratulations on your successful purchase of the alarm system!", 3000);
                    }
                    return;
                }
        ...
        private void OnFurnitureBuy(ExtPlayer player, string name, int type = 0)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be at home or at the store for this action", 3000);
                    return;
                }
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Your apartment already has too much furniture, sell something", 3000);
                    return;
                }
                ...
                if(furnitureData.Key != null)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "There can only be one burglary-proof safe in the house.", 3000);
                        return;
                    }
                    ...
                    if (type == 0)
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have successfully purchased {name}", 3000);
                    }
                    else
                    {
                        foreach (var itemData in furnitureData.Value.Items)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You don't have enough {Chars.Repository.ItemsInfo[itemData.Key].Name} {itemData.Value - count} шт.", 5000);
                                return;
                            }
                        }
                        foreach (var itemData in furnitureData.Value.Items)
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have successfully crafted {name}", 3000);
                    }
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "There was a mistake in the choice of furniture", 3000);
        ...
        private void OnHouseFurnitureSell(ExtPlayer player, int id, int type)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have no furniture", 3000);
                    return;
                }

                if (!houseFurniture.ContainsKey(id))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Error in furniture selection, contact the technical section.", 3000);
                    return;
                }
                ...
                switch (type)
                {
                    case 0:
                        if (furniture.IsSet)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Remove the furniture before you sell it.", 3000);
                            return;
                        }
                        ...
                        if (FurnitureManager.NameModels.ContainsKey(furniture.Name))
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You successfully sold {furniture.Name} for {MoneySystem.Wallet.Format(price)}$", 3000);
                            MoneySystem.Wallet.Change(player, price);
                            house.DestroyFurniture(furniture.Id);
                            houseFurniture.Remove(id);
                            
                            if(FurnitureManager.NameModels[furniture.Name].Type.Equals("Repositories")) 
                    ...
                    case 1:
                        if (furniture.IsSet)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can't remove furniture while there are items in it.", 3000);
                                return;
                            }
                        ...
                        else
                        {
                            if (sessionData.HouseData.Editing)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must finish editing", 3000);
                                return;
                            }
        ...
        private void OnResidentDell(ExtPlayer player, string name)
        {
            try
            {
                ...
                if (house.Roommates.ContainsKey(name))
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You evicted {name} from his home", 3000);
        ...
        public void VehicleTuning(ExtPlayer player, ExtVehicle vehicle)
        {
            ...
            if (vehicleData == null)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You can't drive this car into the garage", 3000);
                return;
            }
            ...
            try
            {
                ...
                switch (action)
                {
                    case "tune": 
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can repair a car near an NPC", 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have to be near the parking lot", 3000);
                            return;
                        }
                    ...
                    case "spawn":
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Your helicopter is waiting for you on the takeoff pad.", 3000);
                        return;
                    case "sell":
                        if (vehicleData.Holder != sessionData.Name)
                        {
                        ...
                        Trigger.ClientEvent(player, "openDialog", "CAR_SELL_TOGOV", $"Do you really want to sell to the state {vehicleData.Model} ({number}) for ${MoneySystem.Wallet.Format(price)}?");
                        return;
                    case "repair":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Machine. {vehicleData.Model} ({number}) was taken to the impound lot", 3000);
                            return;
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Machine. {vehicleData.Model} ({number}) does not need to be restored", 3000);
                            return;
                        }
                        ...
                        vehicleData.Health = 1000;
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You have restored {vehicleData.Model} ({number})", 3000);
                        return;
                    case "evac":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Insufficient funds (not enough {Main.EvacCar - characterData.Money}$)", 3000);
                            return;
                        }
                        ...
                        else
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must be near the garage", 3000);
                                return;
                            }

                            if (garage.IsGarageToNumber(vehicleData.SqlId))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This car is parked in the garage", 3000);
                                return;
                            }
                        }
                        vehiclePlayer = VehicleData.LocalData.Repository.GetVehicleToNumber(VehicleAccess.Personal, number);
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't tow a car.", 3000);
                            return;
                        }                   
                        if (isAir)
                        {
                            if (VehicleModel.AirAutoRoom.IsVehicleToSpawn (vehiclePlayer.Position))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"the vehicle is already parked in the parking lot", 3000);
                                return;
                            }
                        }
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can't tow a car while someone is in it.", 3000);
                            return;
                        }
                        ...
                        if (isAir)
                        {
                            ...
                            vehiclePlayer = VehicleStreaming.CreateVehicle(NAPI.Util.GetHashKey(vehicleData.Model), positionData.Item1, positionData.Item2.Z, 0, 0, number, petrol: vehicleData.Fuel, acc: VehicleAccess.Personal, locked: true, dirt: vehicleData.Dirt);
                            VehicleManager.ApplyCustomization(vehiclePlayer);
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Your helicopter is evacuated", 3000);
                        }
                        else
                        {
                            ...
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Your car has been taken to a garage", 3000);
                        }
                    ...
                    case "gps":
                        if (!isAir)
                        {
                            ...
                            if (garage.IsGarageToNumber(vehicleData.SqlId))
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    "This car is parked in the garage", 3000);
                                return;
                            }
                        }
                        ...
                        if (Ticket.IsVehicleTickets(vehicleData.SqlId))
                        {
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                "Your car is in an impound lot. Marked on the map.", 3000);
                        }
                        else
                        {
                            vehiclePlayer = VehicleData.LocalData.Repository.GetVehicleToNumber(VehicleAccess.Personal, number);
                            if (vehiclePlayer == null)
                            {
                                if (isAir)
                                {
                                    ...
                                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                        "The location of the t/c call was noted in the GPS", 3000);
                                }
                                return;
                            }
                            ...
                            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                                "The location of your car was noted in the GPS", 3000);
                        }
                    ...
                    case "key":
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You received the key to the car with the license plate {number}", 3000);
                    ...
                    case "changekey":
                        ...
                        if (!isAir)
                        {
                            if (garage?.Type == -1)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        $"You must be near the garage", 3000);
                                    return;
                                }
                            }
                            else if (garage?.Type == 6)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                        $"You must be near the parking space", 3000);
                                    return;
                                }
                            }
                            else
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Changing the locks costs $100.", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You changed the locks on your car {number}. Now the old keys cannot be used", 3000);
                        return;
                }
            }
            catch (Exception e)
        ...
        public static void InviteToRoom(ExtPlayer player, ExtPlayer guest)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not available at this time.", 3000);
                    return;
                }
                if (characterData.AdminLVL != 9 && house.Roommates.Count >= MaxRoommates[house.Type])
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have the maximum number of players living in your house", 3000);
                    return;
                }
                if (GetHouse(guest) != null)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player is already living in the house", 3000);
                    return;
                }
                if ((guestSessionData.SellItemData.Buyer != null || guestSessionData.SellItemData.Seller != null) && Chars.Repository.TradeGet(guest))
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Not available at the moment.", 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(guest, "openDialog", "ROOM_INVITE", $"Player ({player.Value}) suggested that you move in with him");

                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered a player ({guest.Value}) to move in with you", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void acceptRoomInvite(ExtPlayer player)
        {
            try
            {
                ...
                if (house == null || house.Type == 7)
                {
                    ...
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player has no personal home", 3000);
                    return;
                }
                if (ownerCharacterData.AdminLVL != 9 && house.Roommates.Count >= MaxRoommates[house.Type])
                {
                    ownerSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Maximum number of players live in the house", 3000);
                    return;
                }
                if (house.Roommates.ContainsKey(player.Name))
                {
                    ownerSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have already been moved into this house.", 3000);
                    return;
                }
                ...
                Notify.Send(owner, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) moved in with you", 3000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You moved in with a player ({owner.Value})", 3000);
        ...
        public static async void CMD_CreateHouse(ExtPlayer player, int type, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Wrong type", 3000);
                    return;
                }
        ...
        public static void CMD_InvitePlayerToHouse(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are too far away", 3000);
                    return;
                }
        ...
        public static void InvitePlayerToHouse(ExtPlayer player, ExtPlayer guest)
        {
            try
            {
                ...
                Notify.Send(guest, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) invited you into his home", 3000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You invited a player ({guest.Value}) into your home", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void OfferHouseSell(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are too far away from the customer", 3000);
                    return;
                }
                var house = GetHouse(player, true);
                if (house == null)
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The player already has real estate", 3000);
                    return;
                }
                ...
                if (house.Type <= 2)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The price should be at least {lowprice} and no more {highprice}", 5000);
                        return;
                    }
                }
                else if (price < lowprice)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The price should be at least {lowprice}", 5000);
                    return;
                }
                if (player.Position.DistanceTo(house.Position) > 30)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You are too far away from the property", 3000);
                    return;
                }
                ...
                Trigger.ClientEvent(target, "openDialog", "HOUSE_SELL", $"Player ({player.Value}) offered you to buy your real estate on the {maxCarsAmount} garage spaces for ${MoneySystem.Wallet.Format(price)}");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You offered a player ({target.Value}) Buy your property on {maxCarsAmount} garage spaces for ${MoneySystem.Wallet.Format(price)}", 5000);
            }
            catch (Exception e)
            {
        ...
        public static void acceptHouseSell(ExtPlayer player)
        {
            try
            {
                ...
                if (GetHouse(player, true) != null)
                {
                    ...
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You already have real estate", 3000);
                    return;
                }
                ...
                if (MoneySystem.Bank.GetBalance(house.BankID) < (tax * 2))
                {
                    sellerSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The seller must pay taxes at least 2 hours before selling the property to you.", 4000);
                    return;
                }
                ...
                if (vehiclesCount > GarageManager.GarageTypes[GarageManager.Garages[house.GarageID].BDType].MaxCars)
                {
                    sellerSessionData.SellItemData = new SellItemData();
                    sessionData.SellItemData = new SellItemData();
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The property you want to buy has a garage for fewer cars.", 3000);
                    return;
                }
                ...
                house.SetOwner(player.Name);
                Notify.Send(seller, NotifyType.Info, NotifyPosition.BottomCenter, $"Player ({player.Value}) Bought real estate from you", 3000);
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You bought real estate from a player ({seller.Value})", 3000);
