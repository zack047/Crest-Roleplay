    ...
    class GarageManager : Script
    {
        ...
        public static void CMD_SetGarage(ExtPlayer player, int ID)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have to stand on the real estate marker", 3000);
                    return;
                }
        ...
        public static void CMD_TpHouse(ExtPlayer player, int id)
        {
            try
            {
                ...
                Admin.AdminsLog(characterData.AdminLVL, $"{player.Name.Replace('_', ' ')} ({characterData.UUID}) teleported to house {id}", 2, "#D289FF", true, hideAdminLevel: 6);
                GameLog.Admin($"{player.Name.Replace('_', ' ')} ({characterData.UUID})", $"teleported to house ({id})", $"");
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have teleported to house ({id})", 4000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_tpBiz(ExtPlayer player, int id)
        {
            try
            {
                ...
                Trigger.Dimension(player, 0);
                Admin.AdminLog(characterData.AdminLVL, $"{player.Name.Replace('_', ' ')} ({characterData.UUID}) teleported to business {id}");
                GameLog.Admin($"{player.Name.Replace('_', ' ')} ({characterData.UUID})", $"teleported to business ({id})", $"null");
            }
            catch (Exception e)
            {
        ...
        public static void CMD_RemoveGarage(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You must stand on the garage marker", 3000);
                    return;
                }
