...
namespace NeptuneEvo.Houses
{
    class Hotel : Script
    {
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                foreach (Vector3 pos in HotelEnters)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel("~w~Hotel", pos + new Vector3(0, 0, 0.5), 5f, 0.4f, 0, new Color(255, 255, 255));
                ...
                foreach (Vector3 pos in CarsGet)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pos - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1f, new Color(255, 255, 255, 220));
                    NAPI.TextLabel.CreateTextLabel("~w~Hotel Scooter", pos + new Vector3(0, 0, 0.5), 5f, 0.4f, 0, new Color(255, 255, 255));
                    HotelID++;
                }
            }
            catch (Exception e)
        ...
        public static void OnEnterHotel(ExtPlayer player, int Index)
        {
            ...
            else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have already rented a hotel", 3000);
        }
        ...
        public static void OnCarRentHotel(ExtPlayer player, int Index)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Cannot be used while sitting in the car.", 3000);
                return;
            }
            if (Index == characterData.HotelID)
            {
                if (sessionData.HotelData.Car != null)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "You successfully gave the scooter away.", 3000);
        ...
        public static void ExtendHotelRent(ExtPlayer player, int hours)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You are not checked into any hotel", 3000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have renewed your lease for {hours}, you will be evicted in {characterData.HotelLeft} hours", 10000);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (listItem)
                {
                    case "rent":
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You live in a house and cannot rent a hotel room", 3000);
                            return;
                        }
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You have rented a hotel room for 1 hour. You can extend the lease in the phone (M)", 3000);
        ...
        private static void callback_hotelmanage(ExtPlayer player, object listItem)
        {
            try
            {
                ...
                {
                    case "extend":
                        Trigger.ClientEvent(player, "openInput", $"Extend the lease ({Main.HotelRent}$/h)", "Enter the number of hours", 1, "extend_hotel_rent");
                        return;
                    case "moveout":
