    ...
    public class SafeMain : Script
    {
        ...
        public static void startSafeDoorCracking(ExtPlayer player)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You don't have a burglary drill", 3000);
                    return;
                }
                if (isCracking)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The break-in has already begun", 3000);
                    return;
                }
                if (isOpen)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"The door to the vault is already open", 3000);
                    return;
                }
                ...
                canBeClosed = false;
                Manager.sendFractionMessage((int)Fractions.Models.Fractions.CITY, "Someone is trying to break down the door to the City Hall vault.");
                Manager.sendFractionMessage((int)Fractions.Models.Fractions.POLICE, "Someone is trying to break down the door to the City Hall vault.");
                Manager.sendFractionMessage((int)Fractions.Models.Fractions.SHERIFF, "Someone is trying to break down the door to the City Hall vault.");
                Manager.sendFractionMessage((int)Fractions.Models.Fractions.FIB, "Someone is trying to break down the door to the City Hall vault.");
                Manager.sendFractionMessage((int)Fractions.Models.Fractions.ARMY, "Someone is trying to break down the door to the City Hall vault.");
            }
            catch (Exception e)
            {
        ...
        public void onPlayerEnterVehicle(ExtPlayer player, ExtVehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                if ((Bags == ItemId.BagWithDrill || Bags == ItemId.BagWithMoney) && player.VehicleSeat == (int)VehicleSeat.Driver)
                {
                    ...
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You cannot sit in the driver's seat with {Chars.Repository.ItemsInfo[Bags].Name}.", 3000);
                }
        ...
        public void SafeCracker_Init()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("Click \n'Interaction'\n~g~Caleb Baker"), new Vector3(-2.1323678, -1821.9778, 29.543238), 5F, 0.8F, 0, new Color(255, 255, 255));

                for (int b = 0; b < salutePoints.Count; b++)
                {
                    PedSystem.Repository.CreateQuest("ig_ramp_hic", salutePoints[b], 105.93525f, 0, title: "~y~NPC~w~ Sergey\nFireworks vendor", colShapeEnums: ColShapeEnums.SaluteShop);
                    //CustomColShape.CreateCylinderColShape(salutePoints[b], 1.5F, 2, 0, ColShapeEnums.SaluteShop);

                    Main.CreateBlip(new Main.BlipData(654, "Salute", salutePoints[b], 32, true));
        ...
        public static void openSafe(ExtPlayer player, params object[] arguments)
        {
            try
            {
                ...
                {
                    Trigger.ClientEvent(player, "dial", "close");
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Incorrect password", 2000);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You've picked up {stage} of 3 passwords", 2000);
        ...
        public static void OnHouseSafe(ExtPlayer player, int _)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Hacking is only possible when wearing a mask", 3000);
                        return;
                    }
                    if (safe.Occupier != null && NAPI.Player.GetPlayerFromHandle(safe.Occupier) != null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"This safe is already being broken into", 3000);
                        return;
                    }
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Available for gangs only", 5000);
                        return;
                    }
                    ...
                    if (count == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You need a lockpick to break in. You can buy it on the black market. (GPS-Miscellaneous)", 5000);
                        return;
                    }
                    if (DateTime.Now < NextRobbery && NowRobberyID != safe.ID)
                    {
                        ...
                        if (g.Hour >= 1) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Try through {g.Hour}:{g.Minute}:{g.Second}", 3000);
                        else Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Try through {g.Minute}:{g.Second}", 3000);
                        return;
                    }
                    ...
                    if (gangsters == 0)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You need at least one more person to get started.", 3000);
                        return;
                    }
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"The cops will be here any minute.", 3000);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.POLICE, "!{#F08080}[F] " + $"Safe No.{safe.ID} at {safe.Address} trying to break in.", true);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.SHERIFF, "!{#F08080}[F] " + $"Safe No.{safe.ID} at {safe.Address} trying to break in.", true);
                    Manager.sendFractionMessage((int)Fractions.Models.Fractions.FIB, "!{#F08080}[F] " + $"Safe No.{safe.ID} at {safe.Address} trying to break in.", true);
                    ...
                    if (player.HasSharedData("IS_MASK") && !player.GetSharedData<bool>("IS_MASK"))
                    {
                        WantedLevel wantedLevel = new WantedLevel(4, LangFunc.GetText(LangType.Ru, DataName.Police), DateTime.Now, "Safe robbery");
        ...
        public static void MoneyFlow(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You've laundered {(int)(Convert.ToInt32(Bags.Data) * 0.97)}$. The man took {(int)(Convert.ToInt32(Bags.Data) * 0.03)}$ for their services", 3000);
            }
            catch (Exception e)
            {
        ...
        public static void CMD_RemoveSafe(ExtPlayer player)
        {
            try
            {
                ...
                if (sessionData.TempSafeID == -1)
                {
                    Trigger.SendChatMessage(player, "~r~[Error] ~w~You should be near the safe.");
                    return;
                }
            ...
            try
            {
                ...
                switch (index)
                {
                    ...
                    case 2:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a lock picker", 3000);
                        return;
                    case 3:
                    ...
                    case 5:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a bag over your head", 3000);
                        return;
                    case 6:
                    ...
                    case 7:
                        ...
                        Trigger.ClientEvent(player, "openDialog", "CONFIRM_BUY_BODYARMOUR", "Do you really want to craft a vest worth 150 materials?");
                        return;
                    case 69:
                        if (!sessionData.CuffedData.Cuffed)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You're not wearing handcuffs.", 3000);
                            return;
                        }
                    ...
                    case 79:
                        ...
                        { 
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You already have a medical card.", 3000); 
                            return; 
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a medical card.", 3000); 
                    ...
                    case 80:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a fake QR code", 5000);
                        return;
                    case 81:
                        if (Chars.Repository.isItem(player, "inventory", ItemId.RadioInterceptor) != null) 
                        { 
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You already have a Radio Interceptor.", 5000); 
                            return; 
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a Radio Interceptor", 5000);
                        return;
                    default:
            ...
            try
            {
                ...
                switch (index)
                {
                    case 70:
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought fireworks", 3000);
    ...
    public class Safe
    {
        ...
        public void Create()
        {
            try
            {
                ...
                Label = (ExtTextLabel) NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Safe"), Position + new Vector3(0, 0, 1.05), 5f, 0.65f, 0, new Color(255, 255, 255), false);
