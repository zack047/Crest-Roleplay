    ...
    class ReportSys : Script
    {
        ...
        public static void ReportTake(ExtPlayer player, int id)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You have already taken one report.", 3000);
                    return;
                }
        ...
        public static void ReportSend(ExtPlayer player, int ID, string answer)
        {
            try
            {
                ...
                {
                    Trigger.SendChatMessage(player, "This complaint is no longer available for modification.");
                    Remove(ID, player);
                }
            }
            catch (Exception e)
        ...
        public static void AddReport(ExtPlayer player, string question, string name)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "The question contains more than 150 characters, ask a shorter question.", 5000);
                    return;
                }
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Your question {Reports.Count + 1} in line.", 5000);
                ...
                if (Main.AllAdminsOnline.Count <= 2) 
                    Notify.Send(player, NotifyType.Alert, NotifyPosition.BottomCenter, "At this time the administration may answer a little longer than usual, we apologize for the possible wait and thank you for your understanding.", 8000);
                            ...
                            try
                            {
                                ...
                                if (Reports.Count >= 1)
                                {
                                    foreach (ExtPlayer foreachPlayer in Main.AllAdminsOnline)
                                    {
                                        ...
                                        NAPI.Notification.SendNotificationToPlayer(foreachPlayer, $"In the system ~r~{Reports.Count} ~s~reports!", true);
        ...
        private static void AddAnswer(ExtPlayer player, int repID, string response)
        {
            try
            {
                ...
                if (response.Length >= 300)
                {
                    ...
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "The answer is too long (more than 300 characters).", 3000);
                    return;
                }
                ...
                else
                {
                    ...
                    EventSys.SendCoolMsg(target,"Response to report", player.Name, $"{response}", "", 10000);
                    //Trigger.ClientEvent(target, "StartDangerButtonSound_client", "sounds/icq.mp3");
                    
                    Trigger.SendToAdmins(Main.ServerSettings.MinAdminLvlReport, $"{ChatColors.Report}[A][{responsetime.ToString("mm\\:ss")}] {player.Name}({player.Value}) for {target.Name}({target.Value}): {response}");
                }
