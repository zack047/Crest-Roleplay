    ...
    class BusinessManager : Script
    {
        ...
        public static void StartTuning(ExtPlayer player)
        {
            try
            {
                ...
                if (vehicleLocalData.Access == VehicleAccess.Personal)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Air transport cannot be tuned up", 3000);
                        return;
                    }
        ...
        public static void ClothesBuy(ExtPlayer player, string name, int id, int texture, bool isDonate = false)
        {
            try
            {
                ...
                EventSys.SendCoolMsg(player,"Purchase", "Buying clothes", $"{LangFunc.GetText(LangType.Ru, DataName.BuyNewClothes)}", "", 6000);
        ...
        public static void fillCar(ExtPlayer player, int lvl)
        {
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    ...
                    Commands.RPChat("sme", player, $"tucked in" + (characterData.Gender ? "" : "а") + LangFunc.GetText(LangType.Ru, DataName.veh));
                }
            }
            catch (Exception e)
            {
        ...
        public static void sellBusinessCommand(ExtPlayer player, ExtPlayer target, int price)
        {
            try
            {
                ...
                EventSys.SendCoolMsg(player,"Предложение", "Purchase", $"{LangFunc.GetText(LangType.Ru, DataName.BizWantSellToYou, target.Value, MoneySystem.Wallet.Format(price))}", "", 10000);
        ...
        public static void acceptBuyBusiness(ExtPlayer player)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"You bought from {target.Name.Replace('_', ' ')} {BusinessTypeNames[biz.Type]} for {MoneySystem.Wallet.Format(price)}$", 3000);
                Notify.Send(target, NotifyType.Info, NotifyPosition.BottomCenter, $"{player.Name.Replace('_', ' ')} bought from you {BusinessTypeNames[biz.Type]} for {MoneySystem.Wallet.Format(price)}$", 3000);
            ...
            try
            {
                ...
                foreach (var p in biz.Products)
                {
                    ...
                    if (busProductData.ItemId != ItemId.Debug)
                    {
                        ...
                        var productData = new Fractions.Manager.FracMatsData(index, itemInfo.Name, itemInfo.Icon, $"{p.Price}$", itemInfo.Name, (int)busProductData.ItemId);
                        jsonData.Add(productData);
                        //Console.WriteLine($"Product: {productData.Name}, Icon: {productData.Icon}, Price: {productData.Price}, RealName: {productData.RealName}, ItemId: {productData.ItemId}");
                    }
                    else if (p.Name == "Сим-карта")
                    {
                        var productData = new Fractions.Manager.FracMatsData(index, "Сим-карта", "sm-icon-sim", $"{p.Price}$", "SIM Card");
                        jsonData.Add(productData);
                        //Console.WriteLine($"Product: {productData.Name}, Icon: {productData.Icon}, Price: {productData.Price}, RealName: {productData.RealName}");
                    }
                    else if (p.Name == "Лотерейный билет")
                    {
                        var productData = new Fractions.Manager.FracMatsData(index, "Лотерейный билет", "sm-icon-lotary", $"{p.Price}$", "Lottery Ticket");
                        jsonData.Add(productData);
                        //Console.WriteLine($"Product: {productData.Name}, Icon: {productData.Icon}, Price: {productData.Price}, RealName: {productData.RealName}");
                    }
                    index++;
        ...
        public static void Event_ShopCallback(ExtPlayer player, int index)
        {
            try
            {
                ...
                if (prod.Name == "Сим-карта")
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a SIM card with a number {number}. It has been added to your inventory", 3000);
                    //Chars.Repository.PlayerStats(player);
                ...
                else if (prod.Name == "Лотерейный билет")
                {
                    ...
                    if (!Lottery.LotteryBought.ContainsKey(mynumb))
                    {
                        ...
                        switch (mynumb)
                        {
                            ...
                            case 3001:
                            case 5001:
                                NAPI.Chat.SendChatMessageToAll($"~g~[LOTTERY] Sold over {mynumb - 1} lottery tickets. Jackpot amount: {Wallet.Format(Lottery.Price)}$!");
                                NAPI.Chat.SendChatMessageToAll($"~g~[LOTTERY] Tickets are on sale at all stores 24/7.");
                                break;
                            default:
                                break;
                        }
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You bought a lottery ticket #{mynumb}. Additional information: /lottery", 7000);
                ...
                else
                {
                    ...
                    if (busProductData.ItemId != ItemId.Debug)
                    {
                        ...
                        EventSys.SendCoolMsg(player,"Purchase", "Item purchase", $"{LangFunc.GetText(LangType.Ru, DataName.YouBuy, "an Item!", prod.Price)}", "", 6000); 
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"{LangFunc.GetText(LangType.Ru, DataName.YouBuy, "an Item!", prod.Price)}", 1000);
        ...
        public static void OpenPetrolMenu(ExtPlayer player)
        {
            try
            {
                ...
                EventSys.SendCoolMsg(player,"Refueling", "Welcome!", $"Price per liter: {prod.Price}$", "", 10000);
        ...
        public static void Event_WShopComponent(ExtPlayer player, int cat, int index, string componentId)
        {
            try
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You do not have a gun license. You can get one from the police department.", 10000);
                    return;
                }
                Business biz = BizList[bizid];
