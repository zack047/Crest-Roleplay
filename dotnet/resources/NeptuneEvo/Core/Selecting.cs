    ...
    class Selecting : Script
    {
        ...
        public static void BeltSelected(ExtPlayer player, bool toggle)
        {
            try
            {
                var characterData = player.GetCharacterData();
                if (characterData == null) return;
                if (!toggle) Commands.RPChat("sme", player, $"buckled" + (characterData.Gender ? "s" : "s") + " seat belt");
                else Commands.RPChat("sme", player, $"unbuttoned" + (characterData.Gender ? "s" : "s") + " seat belt");
            ...
            try
            {
                ...
                switch (index)
                {
                    ...
                    case 1: 
                        ...
                        {
                            Commands.RPChat("sme", player, $"closed" + (characterData.Gender ? "s" : "s") + " trunk");
                        ...
                        else
                        {
                            ...
                            if (vehicleLocalData1 != null)
                            {
                                ...
                                Commands.RPChat("sme", player, $"opened" + (characterData.Gender ? "s" : "s") + " trunk");
                            }
                        }
                        return;
                    case 2:
                                ...
                                try
                                {
                                    ...
                                    if (random >= 85)
                                    {
                                        ...
                                        if (Chars.Repository.ItemsData.ContainsKey(locationName))
                                        {
                                            ...
                                            if (item != null)
                                            {
                                                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"You successfully hacked into the car and found $600 in it, an item, and also started it.", 8000);
                    ...
                    case 10:
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            Commands.RPChat("sme", player, $"accessed" + (characterData.Gender ? "" : "la") + $" in the trunk");
                    ...
                    case 11:
                        ...
                        if (vehicleLocalData != null)
                        {
                            ...
                            Commands.RPChat("sme", player, $"pulled out" + (characterData.Gender ? "" : "") + $" {target.Name} from the trunk");
                        }
                        return;
                    case 12:
                        vehicleLocalData = vehicle.GetVehicleLocalData();
                        if (vehicleLocalData != null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, "You can change only in your car", 3000);
                                return;
                            }

                            if (sessionData.ActiveWeap.Item == null || sessionData.ActiveWeap.Index == -1)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You must have a license plate in your hand", 2000);
                                return;
                            }
                            ...
                            Trigger.PlayAnimation(player, "mini@safe_cracking", "idle_base", 39);
                            // Trigger.ClientEventInRange(player.Position, 250f, "PlayAnimToKey", player, false, "vzlomsafe");
                            Commands.RPChat("sme", player, "changes numbers");
                            Trigger.ClientEvent(player, "fullblockMove", true);
                            Trigger.ClientEvent(player, "freeze", true);
                            NAPI.Task.Run(() =>
                            {
        ...
        public static void DetachPlayer(ExtPlayer player)
        {
            try
            {
                ...
                player.ResetSharedData("AttachToVehicle");
                Trigger.ClientEvent(player, "setPocketEnabled", false);
                Commands.RPChat("sme", player, $"came out" + (characterData.Gender ? "" : "la") + $" from the trunk");
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case "offer":
                        ...
                        EventSys.SendCoolMsg(target,"Offer", "Exchange", $"{LangFunc.GetText(LangType.Ru, DataName.OfferTrade, player.Value)}", "", 10000);
                        EventSys.SendCoolMsg(player,"Offer", "Exchange", $"{LangFunc.GetText(LangType.Ru, DataName.OfferedTrade, target.Value)}", "", 5000);
        ...
        public static void carryAnimations(ExtPlayer player, ExtPlayer target, int action)
        {
            try
            {
                ...
                else if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath && !targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath && !sessionData.AntiAnimDown && !targetSessionData.AntiAnimDown)
                {
                    ...
                    EventSys.SendCoolMsg(target,"Offer", "Pair animation", $"{LangFunc.GetText(LangType.Ru, DataName.PlayerWants, player.Value, action_name)}", "", 12000);
        ...
        public static void pairedAnimations(ExtPlayer player, ExtPlayer target, string action)
        {
            try
            {
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath && !targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath && !sessionData.AntiAnimDown && !targetSessionData.AntiAnimDown)
                {
                    ...
                    if (action == "PAIRED_FIVE" || action == "PAIRED_SLAP") EventSys.SendCoolMsg(target,"Offer", "Pair animation", $"{LangFunc.GetText(LangType.Ru, DataName.PlayerWantInteractYou, player.Value, action_name)}", "", 10000);
                    ...
                    else  EventSys.SendCoolMsg(target,"Offer", "Pair animation", $"{LangFunc.GetText(LangType.Ru, DataName.YouWantInteract, player.Value, action_name)}", "", 10000);
        ...
        public static void playerTransferMoney(ExtPlayer player, string arg)
        {
            try
            {
                ...
                Commands.RPChat("sme", player, $"forwarded by" + (characterData.Gender ? "" : "а") + $" {MoneySystem.Wallet.Format(amount)}$ " + "{name}", target);
            ...
            try
            {
                ...
                if (!sessionData.CuffedData.Cuffed && !sessionData.DeathData.InDeath)
                {
                    if (!targetSessionData.CuffedData.Cuffed && !targetSessionData.DeathData.InDeath)
                    {
                        ...
                        EventSys.SendCoolMsg(target,"Offer", "Handshake!", $"{LangFunc.GetText(LangType.Ru, DataName.HandshakeYou, player.Value)}", "", 10000);
                        ...
                        EventSys.SendCoolMsg(player,"Offer", "Handshake!", $"{LangFunc.GetText(LangType.Ru, DataName.YouHandshake, target.Value)}", "", 10000);
                    }
