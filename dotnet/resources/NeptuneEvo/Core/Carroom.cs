    ...
    class CarRoom : Script
    {
        ...
        public static void RemoteEvent_carroomBuy(ExtPlayer player, string vName, string color, int id)
        {
            try
            {
                ...
                if (id == 1) // Для себя
                {
                    ...
                    if (prod == null)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "There was an error, the transport model is out of stock.", 3000);
                        RemoteEvent_carroomCancel(player);
                        return;
                    }
                    ...
                    if (house != null)
                    {
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You have the maximum number of cars that supports your residence.", 3000);
                            RemoteEvent_carroomCancel(player);
                            return;
                        }
                    }
                    if (biz.IsOwner())
                    {
                        if (biz.Type == 15)
                        {
                            if (prod.Lefts != 0)
                            {
                                ...
                                {
                                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "This model is no longer in stock", 3000);
                                    RemoteEvent_carroomCancel(player);
                                    return;
                                }
                ...
                else if (id == 2) // В организацию
                {
                    ...
                    if (organizationData.Vehicles.Count >= maxcars)
                    {
                        ...
                        if (upgraded <= 1) Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"You can increase the number of vehicles by buying an improvement for the family!", 6000);
                        RemoteEvent_carroomCancel(player);
                        return;
                    }
        ...
        private static void OnAutoroomBuy(ExtPlayer player, string vName, string color, int id, bool isDonate)
        {
            try
            {
                ...
                if (id == 1) // Для себя
                {
                    if (!VehicleModel.AirAutoRoom.isAirCar(vName))
                    {
                        ...
                        if (house != null)
                        {
                            ...
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                                    $"You have the maximum number of cars that supports your residence.",
                                    3000);
                                RemoteEvent_carroomCancel(player);
                                return;
                            }
                ...
                else if (id == 2) // В организацию
                {
                    ...
                    if (VehicleModel.AirAutoRoom.isAirCar(vName))
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "You cannot buy this vehicle into the organization.", 3000);
                        return;
                    }
        ...
        public static void timer_exitVehicle(ExtPlayer player, bool quit = false)
        {
            if (OnExitTestDrive (player) && !quit)
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"The time allotted for the test drive is over.", 3000);
        }
        ...
        public void onPlayerExitVehicleHandler(ExtPlayer player, ExtVehicle vehicle)
        {
            if (OnExitTestDrive (player))
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"The test drive was over as you left the vehicle.", 3000);
