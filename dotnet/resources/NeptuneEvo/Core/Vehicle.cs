    ...
    class VehicleManager : Script
    {
        ...
        public static void Init()
        {
            try
            {
                ...
                Log.Write($"Vehicles are loaded ({vehiclesData.Count}) | Rescheduled item - {countSwipe} piece.", nLog.Type.Success);
            }
            catch (Exception e)
            {
            ...
            try
            {
                ...
                if (vehicleLocalData != null)
                {
                    if (player.VehicleSeat == (int)VehicleSeat.Driver)
                    {
                        ...
                        // Set engine state to off when player attempts to exit the vehicle
                        VehicleStreaming.SetEngineState(vehicle, false);
                    }
                    ...
                    if (!VehicleModel.AirAutoRoom.isAirCar(Model))
                    {
                        NAPI.Task.Run(() =>
                        {
                            ...
                            if (house != null && Houses.GarageManager.Garages.ContainsKey(house.GarageID))
                            {
                                ...
                                EventSys.SendCoolMsg(player,"Transportation", "Purchase of transport", $"{LangFunc.GetText(LangType.Ru, DataName.VehSoonGarage)}", "", 12000);
                                ...
                                EventSys.SendCoolMsg(player,"Transportation", "Purchase of transport", $"{LangFunc.GetText(LangType.Ru, DataName.VehWhenHome)}", "", 12000);
                ...
                if (vehicleLocalData != null && vehicle.Health < 1f)
                {
                    ...
                    switch (vehicleLocalData.Access)
                    {
                        ...
                        /*ca  se "FRACTION":
                            ...
                            {
                                Chars.Repository.RemoveAll(GetVehicleToInventory(vehicle.NumberPlate));
                                Manager.sendFractionMessage(data.Fraction, $"~o~Багажник фракционного транспорта с номерами {vehicle.NumberPlate} был утерян из-за destroyedия.", true);
                            }
                            return;
                        case "ORGANIZATION":
                            if (Chars.Repository.getCountItems(GetVehicleToInventory(vehicle.NumberPlate)) >= 1)
                            {
                                Chars.Repository.RemoveAll(GetVehicleToInventory(vehicle.NumberPlate));
                                Manager.sendOrganizationMessage(data.Fraction, $"~o~Багажник организационного транспорта с номерами {vehicle.NumberPlate} был утерян из-за destroyedия.", true);
                            }
                            return;*/
                        case VehicleAccess.Garage:
                        case VehicleAccess.Personal:
