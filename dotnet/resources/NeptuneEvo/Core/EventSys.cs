...
namespace NeptuneEvo.Core
{
    class EventSys : Script
    {
        ...
        [Command(AdminCommands.Createmp, "Use: /createmp [Participant Limit] [Zone Radius] [Event Name]", GreedyArg = true)]
        ...
        [Command(AdminCommands.Stopmp, "Use: /stopmp [Winner ID] [Prize #1] [Prize #2?] [Prize #3?")] // ne vishlo
        public static void MPReward(ExtPlayer player, int pid, uint reward, int pid2 = -1, uint reward2 = 0, int pid3 = -1, uint reward3 = 0)
        {
            try
            {
                ...
                if (AdminEvent.EventState == 2)
                {
                    ...
                    else Trigger.SendChatMessage(player, "Reward cannot exceed the posted limit: " + AdminEvent.RewardLimit + ".");
                }
                else Trigger.SendChatMessage(player, LangFunc.GetText(LangType.Ru, DataName.MpNotStarted));
            }
            catch (Exception e)
        ...
        [Command(AdminCommands.Mpveh, "Use: /mpveh [Model name] [Color] [Number of machines")]
        ...
        [Command(AdminCommands.mpskin, "Use: /mpskin [ID] [SKIN]")]
        ...
        [Command(AdminCommands.mpskins, "Use: /mpskins [SKIN] [RADIUS]")]
        ...
        [Command(AdminCommands.Mpreward, "Use: /mpreward [New Limit]")]
        ...
        [Command(AdminCommands.Mpkick, "Use: /mpkick [ID player] [Reason]", GreedyArg = true)]
        ...
        [Command(AdminCommands.Banmp, "Use: /banmp [Player ID] [Reason]", GreedyArg = true)]
        ...
        [Command(AdminCommands.Unbanmp, "Use: /unbanmp [player ID]")]
        ...
        [Command(AdminCommands.Mphp, "Use: /mphp [Number of HP]")]
                ...
                if (AdminEvent.EventMembers.Count != 0)
                {
                    lock(AdminEvent.EventMembers)
                    {
                        foreach (ExtPlayer foreachPlayer in AdminEvent.EventMembers.Keys)
                        {
                            ...
                            SendCoolMsg(foreachPlayer,"Event", AdminEvent.Name, $"{LangFunc.GetText(LangType.Ru, DataName.SpsMp)}", "", 10000);
                        }
                    }
                }
