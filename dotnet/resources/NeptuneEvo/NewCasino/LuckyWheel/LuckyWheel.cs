...
namespace NeptuneEvo.NewCasino
{
    class LuckyWheel
    {
        ...
        private static Dictionary<string, string> Names = new Dictionary<string, string>
        {
            ...
            ["veh_zorrusso"] = "Car zorrusso",
            ["veh_xa21"] = "Car xa21",
            ["veh_cheburek"] = "Car cheburek",
            ["veh_btype"] = "Car btype",
            ["veh_stafford"] = "Car stafford",

            ["clothes_neon_shoes"] = "Neon sneakers",
            ["clothes_mask_boll"] = "Bull mask",
            ["clothes_cosmo_legs"] = "Space pants"
        };
        ...
        private void StartWheel(ExtPlayer player, Action callback)
        {
            try
            {
                if (IsRolling)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "At this moment, someone is spinning the wheel of fortune!", 3000);
                    return;
                }
